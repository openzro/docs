...
Source: https://docs.netbird.io/manage/network-routes

---

# Network Routes

Network Routes let you route traffic from openZro peers to private networks without installing the openZro agent on every device. A routing peer forwards packets between your openZro mesh network and your internal networks (LANs, VPCs, data centers).

    

> **Note:** Network Routes require openZro [v0.9.0](https://github.com/openzro/openzro/releases) or later.

> **Note:** By default, Network Routes bypass Access Control rules. Traffic flows freely to routed networks unless you [configure access control explicitly](/manage/network-routes/use-cases/by-configuration/access-control). See [Network Routes caveats](#network-routes-caveats) for details.
...
A routing peer is a openZro device that forwards traffic between the openZro network and a private network. It must have network access to the resources you want to reach.

> **Note:** A network route grants access to the network behind the routing peer, not to the routing peer machine itself. If you need to reach services running on the routing peer, create a separate peer-to-peer [access control policy](/manage/access-control) that targets the routing peer's group as the destination.

### Routing Group

A routing group is a set of routing peers. Each peer in the group routes packets between your routed network and other openZro peers. Using a routing group provides automatic high availability.
...
Disable masquerade when you need source IP visibility for auditing or compliance. In this case, you must configure a return route to your openZro network in your external network infrastructure.
...
> **Note:** Peers must still be connected to the routing peer to use the route.
...
```bash
openzro up --dns-router-interval 30s
```
...
> **Note:** DNS Routes require openZro [v0.28.0](https://github.com/openzro/openzro/releases) or later. Wildcard domains are not supported.

> **Note:** Starting with openZro v0.59.0, the local DNS forwarder port changed from `5353` to `22054` to avoid collisions. The Management Service applies the new port only when all peers in the account run v0.59.0 or newer.

## Managing Network Routes

A network route describes a network you want to connect with your openZro peers. It includes an identifier, network range, routing peer or peer groups, and parameters for priority and masquerading.
...
To avoid single points of failure, we recommend installing openZro on every resource. When that is not feasible, use high availability mode to ensure reliable connections. Multiple openZro peers can serve as routing peers for the same private network.
...
> **Note:** You can add unlimited routing peers to a highly available route. Each connected peer selects one routing peer based on metric priority (lower = higher priority) and connection type (direct or relayed).
...
Disable masquerade when you need source IP transparency or want to manage routing on your external network. The routing peer forwards packets with the original openZro peer IP intact.

This requires configuring your external network router with a return route to your openZro network through the routing peer. Devices without the agent can then communicate with your openZro peers.
...
```shell
root@brys-vm-nbt-ubuntu-01:~# openzro networks ls
Available Networks:
...
```shell
root@brys-vm-nbt-ubuntu-02:~# openzro networks ls
Available Networks:
...
> **Note:** To prevent permission overflow, use dedicated Routing Peers for Network Routes and never use them for Networks. This separation is achievable but easy to overlook during configuration.
