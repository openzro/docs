...
Source: https://docs.netbird.io/manage/settings/multi-factor-authentication

---

# Multi-Factor Authentication (MFA)

openZro supports Multi-Factor Authentication (MFA) to enhance the security of your account by requiring a second form of verification during login.
This works automatically if you have enabled MFA in your SSO Provider (Google, Microsoft etc.) and use the **interactive SSO login** feature in openZro.

openZro MFA is primarily for users with **email / password login**. It is available for all users and can be enabled from the Dashboard on the Settings page [https://your-management.example.com/).

> **Note:** You may not need openZro MFA. Your SSO Provider (Google, Microsoft etc.) may already have MFA enabled. Enabling this setting could result in duplicated MFA checks.

> **Note:** This feature is only available in the cloud version of openZro. Self-Hosted users can use their own Identity Provider for MFA.
...
> **Note:** Currently openZro does not provide recovery codes for MFA. Make sure to have multiple administrators configured to prevent lockout situations.
    If all `Owners` or `Admins` lose access to the Dashboard, you should contact [support@openzro.io](mailto:support@openzro.io) for assistance.
