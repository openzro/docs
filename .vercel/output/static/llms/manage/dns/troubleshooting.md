...
Source: https://docs.netbird.io/manage/dns/troubleshooting

---

# DNS Troubleshooting

This guide helps you diagnose and resolve common DNS issues in openZro. Follow the structured approach below to identify and fix problems quickly.
...
> **Note:** **Quick isolation test**: To check if an issue is caused by openZro DNS, temporarily disable DNS management on the peer:
```bash
openzro down
openzro up --disable-dns
```
If the problem goes away, the issue is in your openZro DNS configuration. If it persists, it's unrelated to openZro DNS.
...
#### Solution A: System Not Using openZro DNS

The OS isn't configured to use openZro's resolver.

**Check**: Refer to the [Verifying Configuration](#verifying-configuration) section to confirm if your OS is using openZro's resolver.

**Fix**:
If the configuration is incorrect, restart openZro to force the OS to pick up the DNS settings:

**Linux/macOS**:
```bash
sudo systemctl restart openzro
# or
sudo openzro down && sudo openzro up
```

**Windows**:
```powershell
Restart-Service openzro
```
...
When Private DNS is enabled, Android bypasses the VPN's DNS configuration and sends DNS queries directly to the configured Private DNS provider, preventing openZro's custom DNS resolution from working.

#### Solution B: Wrong Nameserver Configuration

openZro is routing queries to the wrong DNS server.
...
- If direct resolution works: It's likely a openZro resolver issue.
...
- Review openZro logs: `openzro up --log-level debug`
...
**openZro**:
```bash
# Restart openZro to clear its cache
...
> **Note:** Search domains only work for match domain groups, not primary groups.
...
### Issue 5: DNS Works on openZro Network, Fails Outside

**Symptoms**:
- DNS works when connected to openZro
- DNS fails when openZro disconnects
- Computer can't resolve anything after using openZro
...
# If still shows 100.x.255.254, openZro didn't restore original DNS
```
...
```bash
# Completely remove openZro
sudo openzro down
sudo systemctl stop openzro
sudo systemctl disable openzro

# Restore DNS manually (see above)

# Reinstall openZro
curl -sSL https://github.com/openzro/openzro/releases/latest | sh
```
...
This happens when using openZro DNS with routers that have DNS rebinding protection (pfSense, OpenWRT, etc.).
...
When you configure a nameserver accessible from the Internet (without a VPN), the openZro client acts as a proxy to that public nameserver.

There are two things to verify:

1. The openZro client has picked up the nameserver
2. The operating system is configured to use the openZro client's proxy nameserver

To check the first item (works on all operating systems):

1. Run `openzro status -d`
...
Name                             : openZro-Match
Version                          : 2
Namespace                        : {.openzro.cloud, .83.100.in-addr.arpa}
...
...
Unknown adapter wt0:
   ...
   Connection-specific DNS Suffix  . : openzro.cloud
...
- `100.XXX.255.254` under _NameServers_ (the local proxy address of the openZro client)
- `.openzro.cloud` and `.XXX.100.in-addr.arpa` under _Namespace_ for built-in entries
...
- `100.XXX.255.254` under _nameserver[N]_ (the local proxy address of the openZro client)
- `openzro.cloud` and `.XXX.100.in-addr.arpa` under _domain_ for built-in entries
...
- `127.0.0.1` - default address for openZro DNS proxy listener
- `127.0.0.153` - fallback address for openZro DNS proxy listener
- Value of `$NB_DNS_RESOLVER_ADDRESS` - custom override for the openZro DNS proxy listener

To find the address the openZro client is listening on:

```shell
sudo ss -nlptu 'sport = 53' | grep openzro
sudo netstat -ltnup | grep ':53' | grep openzro
```
...
- The Routing Peer's openZro address is `100.83.136.209`
...
> **Note:** Port `5353` is the well-known Multicast DNS port (used by Avahi, Bonjour, printer sharing, Chromecast, etc.) and may already be in use by other software. This can prevent older Routing Peers from routing Domain Resources.
...
While local DNS forwarder failures are rare, using it is a good way to force the openZro client to set up routing for the domain (see [Domain Resources](/manage/networks#domain-resources) for details).

> **Note:** On macOS and Windows, the IP address is always `100.83.255.254` instead of `100.83.73.97`.
...
```shell
kdn@brys-vm-nbt-ubuntu-01:~$ openzro networks ls
Available Networks:
...
After confirming everything works within openZro's scope, restart openZro and check whether the operating system's default DNS resolver resolves the Domain Resource correctly.

> **Note:** See [Verifying Configuration > Public nameservers](#public-nameservers) for equivalent macOS and Windows debugging steps.

> **Note:** You might notice that `openzro down` followed by `openzro up` doesn't clear the `Resolved IPs`:

  ```shell
  kdn@brys-vm-nbt-ubuntu-01:~$ sudo openzro down
  Disconnected
  kdn@brys-vm-nbt-ubuntu-01:~$ sudo openzro up
  Connected
  kdn@brys-vm-nbt-ubuntu-01:~$ openzro networks ls
  Available Networks:
...
We'll start fresh by restarting the entire openZro service to purge all caches, then proceed with testing:

```shell
kdn@brys-vm-nbt-ubuntu-01:~$ sudo openzro service restart
openZro service has been restarted
kdn@brys-vm-nbt-ubuntu-01:~$ openzro networks ls
Available Networks:
...
> **Note:** The operating system resolver may not be the only source of DNS queries, but using it is required for Domain Resources to work.
