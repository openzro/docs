...
Source: https://docs.netbird.io/manage/dns/internal-dns-servers

---

# Internal DNS Servers

When your network has on-premise DNS infrastructure — Active Directory, BIND, Unbound, or other internal DNS servers — you may need to configure openZro to use them. This page covers when a nameserver configuration is needed and when it isn't.
...
> **Note:** This applies to all domain-based resources, not just Active Directory domains. Any routing peer that needs to resolve internal domain names must have the corresponding nameserver distributed to its group.

## Configuring Nameservers for Internal Domains

To resolve internal domains through openZro, create a **match domain** Nameserver pointing to your internal DNS server:
...
> **Note:** Configure 2-3 nameserver IPs for redundancy. If the first fails, peers automatically try the next one.
...
> **Note:** Each peer should have exactly **one** primary nameserver. If you don't configure a primary, peers use their original DNS settings for non-matched queries.
...
> **Note:** Changes propagate to peers within 10-30 seconds. No peer restart required.
...
> **Note:** Before deleting a primary nameserver, create a new primary first, or peers will revert to their original DNS settings.
...
We do not recommend using Domain Controllers as routing peers. If you must, you need to **disable DNS management** for the DC's group. If openZro manages DNS on a Domain Controller, it can interfere with the AD DNS services that the DC depends on.
...
This prevents openZro from modifying the DC's DNS configuration while still allowing the DC to act as a routing peer. The DC continues using its own DNS for Active Directory services.
...
When openZro is installed on a Windows Domain Controller, the Windows DNS Server service may already be using UDP port 51820 — WireGuard's default port. This causes openZro to fail during startup.

**Symptoms:**
- openZro fails with: `Error: daemon up failed: call service up method: rpc error: code = DeadlineExceeded desc = context deadline exceeded`
- The peer appears as **disconnected** in the openZro management portal
...
3. Optionally, capture a openZro debug bundle for further investigation:

```cmd
openzro debug for 1m -U
```
...
Then restart the openZro service:

```cmd
openzro service restart
```

> **Note:** Restarting the DNS Server service temporarily disrupts DNS resolution for all clients using this Domain Controller. Schedule this change during a maintenance window or ensure other DCs can handle DNS queries.

> **Note:** This port exclusion persists across reboots. If you later change the WireGuard listen port in your openZro configuration, update the exclusion range accordingly.