...
Source: https://docs.netbird.io/manage/team/idp-sync/keycloak-sync
...
openZro's Keycloak integration enhances user management by allowing you to utilize Keycloak as your identity provider.
...
> **Note:** If you are running a self-hosted deployment with a [Commercial License](https://openzro.io/pricing#on-prem) and the embedded IdP, see the [Embedded IdP version](/manage/team/idp-sync/embedded/keycloak-sync) of this guide instead.
...
Once SSO is configured, and you can successfully log in to openZro using your Keycloak credentials, you can proceed with the SCIM setup below.

## Enabling Keycloak SCIM in openZro

To enable SCIM synchronization in openZro, navigate to `Integrations > Identity Provider Sync` in your openZro dashboard.

![openZro Keycloak Integration](/docs-static/img/manage/team/idp-sync/keycloak-sync/keycloak-connect.png)

Click the `Connect Generic SCIM` button to begin the configuration process.

![openZro Keycloak Getting Started](/docs-static/img/manage/team/idp-sync/keycloak-sync/keycloak-scim-getting-started.png)
...
![openZro Keycloak Group Filter](/docs-static/img/manage/team/idp-sync/keycloak-sync/keycloak-scim-group-filter.png)
...
![openZro Keycloak User Group Filter](/docs-static/img/manage/team/idp-sync/keycloak-sync/keycloak-scim-user-group-filter.png)

Click `Continue` to generate your SCIM credentials.

**SCIM Credentials**

openZro will generate the SCIM credentials required to configure Keycloak. Make note of both the **Base URL** and **Token Key** as you will need them in the next section to complete the Keycloak configuration.

![openZro Keycloak SCIM Credentials](/docs-static/img/manage/team/idp-sync/keycloak-sync/keycloak-scim-credentials.png)

Click `Finish Setup` to complete the openZro SCIM configuration.

![openZro Keycloak SCIM Enabled](/docs-static/img/manage/team/idp-sync/keycloak-sync/keycloak-scim-enabled.png)
...
Navigate to the SCIM Administration Console. On the first login screen, enter your realm name (e.g., `openzro`) and click `Start Login`.
...
**Connection Details:**
* **Base URL**: Paste the Base URL you copied from openZro (e.g., `https://api.openzro.io/api/scim/v2`)
...
* **Bearer Token**: Paste the Token Key you copied from openZro
...
Find your realm (e.g., `openzro`) and click `Assign to Realm` to enable SCIM synchronization for that realm.
...
By default, the SCIM provider will synchronize all groups and users from your Keycloak realm to openZro.
To control which specific groups and users should be synchronized, you need to configure resource filtering rules.

Under the `SCIM Client` menu section, click on `Remote SCIM Provider`, then click `Edit` in the openZro provider row.
...
After configuring the SCIM provider and resource filtering, you need to manually synchronize existing users and groups from Keycloak to openZro.
...
After completing the initial sync, you can verify that users and groups have been successfully synchronized by navigating to `Team > Users` in your openZro dashboard.

![openZro Verify Users](/docs-static/img/manage/team/idp-sync/keycloak-sync/openzro-verify-users.png)

> **Note:** SCIM provisioning will manage only resources that are created through Keycloak. Any resources created directly in
    openZro will not be managed by SCIM.
