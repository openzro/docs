...
Source: https://docs.netbird.io/manage/team/idp-sync/google-workspace-sync
...
openZro's Google Workspace integration enhances user management capabilities by synchronizing users and groups from
Google Workspace to openZro. You can utilize these synchronized groups to configure your network, establish network
...
capabilities. GCP is used to create a service account, which openZro uses to authenticate and access Google Workspace
data via the Admin SDK API. This service account uses OAuth 2.0 for secure, authorized access to Workspace information.

> **Note:** If you are running a self-hosted deployment with a [Commercial License](https://openzro.io/pricing#on-prem) and the embedded IdP, see the [Embedded IdP version](/manage/team/idp-sync/embedded/google-workspace-sync) of this guide instead.

## Get Started with openZro-Google Workspace Integration

Go to the `Integrations` section in the left menu to access the `Identity Provider integration`. Click the `Google Workspace` button. This will open a pop-up window featuring an intuitive wizard to guide you through the synchronization process between openZro and Google Workspace.

![openZro Connect openZro with Google Workspace](/docs-static/img/manage/team/idp-sync/google-workspace-sync/q1aq98X.png)

## Prerequisites

The openZro-Google Workspace integration process involves:
...
## Creating the openZro Project

Once you have the necessary permissions, you can create the openZro project in GCP.

![openZro Create openZro Project](/docs-static/img/manage/team/idp-sync/google-workspace-sync/GBbcnt3.png)
...
* Enter `openZro` as the project name.
...
To let `openZro` authenticate and access Google Workspace, you must enable the `Admin SDK API`. Here’s how to do it:
...
## Creating the openZro Service Account

Once you create the project, you can set up the `openZro` service account. On openZro, click `Continue →`. That will show you a summary of the required steps.

![openZro Create Service Account](/docs-static/img/manage/team/idp-sync/google-workspace-sync/tRbBQsR.png)
...
* `openzro` for the service account ID
...
On openZro, click `Continue →`. You’ll need to provide the email of the service account.

![openZro Get Service Account Email](/docs-static/img/manage/team/idp-sync/google-workspace-sync/IBEGQD8.png)
...
Back on openZro, click `Continue →`. You’ll see a summary of how to create a service account key.

![openZro Create Service Account Key](/docs-static/img/manage/team/idp-sync/google-workspace-sync/MGgCzky.png)
...
Now, you can upload the service account key to openZro. After a successful upload, you'll see the key listed in the openZro interface.

![openZro Paste New Key](/docs-static/img/manage/team/idp-sync/google-workspace-sync/g8omqCt.png)

## Creating a New Admin Role in Google Workspace

After creating the service account in GCP and uploading its secret key, return to openZro and click 'Continue →'. The next steps will guide you through creating a role in Google Workspace for this service account

![openZro Create Admin Role](/docs-static/img/manage/team/idp-sync/google-workspace-sync/F80lm4H.png)
...
Fill in the form with the values provided in openZro:
...
Return to openZro and click `Continue →`. The next screen shows the privileges needed for the Admin API.

![openZro Add Role Privileges](/docs-static/img/manage/team/idp-sync/google-workspace-sync/IZn8XWY.png)
...
In openZro, click `Continue →`. For convenience, you can copy the service account Email from this screen and use it to grant it the necessary permissions in Google Workspace.

![openZro Assign Service Account](/docs-static/img/manage/team/idp-sync/google-workspace-sync/jhgTfsk.png)
...
Verify the Email and click `ASSIGN ROLE` to grant the role `User and Group Management ReadOnly` to the `openZro` service account.

![Google Workspace Assign Role](/docs-static/img/manage/team/idp-sync/google-workspace-sync/rPYNLz1.png)

## Entering Customer ID

Go back to openZro and click `Continue →`. The next screen will prompt you to enter your Google Workspace Customer ID.

![openZro Enter Customer ID](/docs-static/img/manage/team/idp-sync/google-workspace-sync/q6tuStz.png)
...
Return to openZro. The next two screens allow you to select which Google Workspace groups and users you want to synchronize. By default, openZro synchronizes all groups and users. If you're okay with syncing everything, click `Continue` on both screens.

![openZro Groups Sync](/docs-static/img/manage/team/idp-sync/google-workspace-sync/gAgKeL0.png)
...
![openZro Users Sync](/docs-static/img/manage/team/idp-sync/google-workspace-sync/6Huo7vW.png)

The next screen, should be similar the following one, verifying that the integration was successful:

![openZro Google Workspace Enabled](/docs-static/img/manage/team/idp-sync/google-workspace-sync/EkPJqpJ.png)
...
![openZro Users](/docs-static/img/manage/team/idp-sync/google-workspace-sync/5AcaIqW.png)
