...
Source: https://docs.netbird.io/manage/team/idp-sync/microsoft-entra-id-sync
...
openZro's Microsoft Entra ID integration allows you to synchronize users and groups from Entra ID to openZro.
You can then use these synchronized groups to configure your network, create network access policies, and automate
onboarding and offboarding processes.

> **Note:** If you are running a self-hosted deployment with a [Commercial License](https://openzro.io/pricing#on-prem) and the embedded IdP, see the [Embedded IdP version](/manage/team/idp-sync/embedded/microsoft-entra-id-sync) of this guide instead.

## Get Started with openZro-Entra ID Integration

To get started, navigate to [Integrations](https://your-management.example.com/) in the left menu, which will take you to the
`Identity Provider` integration. Click the `Entra ID (Azure AD)` button. This action will trigger a pop-up window that will
present you with a user-friendly wizard, guiding you through the synchronization process between openZro and Azure AD.

![openZro Get Started IdP](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/FkdC8BV.png)
...
If you don't have the required permissions, contact your Azure AD administrator to grant you the appropriate role before proceeding with the openZro integration.

## Create and Configure a Microsoft Entra ID Application for openZro Integration

Now that you have the required permissions, return to the openZro dashboard. Click on the `Get Started` button to initiate the integration process.
...
![openZro Create Application](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/oI0Pjai.png)
...
## Configure API Permissions for openZro-Entra ID Integration

On the openZro dashboard click the `Continue →` button. A new wizard screen will appear, this time, offering step-by-step instructions for setting up API permissions.

![openZro Add API Permissions](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/0A98Xm9.png)
...
On the next screen, click on the `Application permissions` button, which will let you select the appropriate permissions for openZro to function correctly with your Microsoft Entra ID environment.
...
The `User.Read.All` permission allows openZro to read the full set of profile properties, group memberships, and reports of the signed-in user and other users in your organization.
...
## Create a Client Secret for Secure openZro-Entra ID Authentication

Back to the openZro dashboard, click the `Continue →` button. A new wizard screen will appear, showing instructions for generating a client secret in Entra ID.

![openZro Generate Client Secret](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/xvLskEg.png)
...
## Enter Application ID and Directory ID in openZro

Paste the secret `Value` from the previous step into openZro and click the `Continue →` button. A new wizard screen will appear, asking for the `Application (client) ID` and the `Directory (tenant) ID` credentials generated previously.

Paste the values and click the `Continue →` button.

![openZro Application ID and Directory](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/6yiGCtY.png)

## Choose Groups to Synchronize from Entra ID
At this stage, openZro is set to synchronize all groups from your Microsoft Entra ID by default. You have two options:
...
![openZro Group Sync](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/xyLPzxH.png)

## Choose Users to Synchronize from Entra ID
After configuring group synchronization, you'll now set up user synchronization. Similar than before, openZro is configured to synchronize all users from your Microsoft Entra ID by default.
...
> **Note:** You can modify these synchronization settings later if necessary.

![openZro Users Sync](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/bpwW1Bn.png)
...
* The integration between openZro and Microsoft Entra ID is active.
* The initial synchronization has been completed successfully.
* openZro will now automatically sync with your Microsoft Entra ID at regular intervals.

> **Note:** You can manually trigger a sync or adjust settings by clicking on the Microsoft Entra ID section in the Identity Provider screen

![openZro Identity Provider Synchronized](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/DH5hxFK.png)

## Verify the Integration

To verify the synchronization, navigate to `Teams > Users` in the left menu.
You should see all the users and groups from your Microsoft Entra ID environment listed in the openZro dashboard.

![openZro Checking Integration](/docs-static/img/manage/team/idp-sync/microsoft-entra-id-sync/qlNlfgV.png)
