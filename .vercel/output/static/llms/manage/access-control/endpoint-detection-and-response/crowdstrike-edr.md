...
Source: https://docs.netbird.io/manage/access-control/endpoint-detection-and-response/crowdstrike-edr
...
The integration of openZro with CrowdStrike Falcon provides organizations with network security controls that allow
...
openZro can ensure that only devices with a high security posture can access the network.

In this guide, we will walk you through the configuration steps to integrate CrowdStrike Falcon with openZro and use ZTA score
...
- Click `Create`
- Copy the credentials. You will need these credentials when configuring an integration in openZro.

## Configure a CrowdStrike Integration in openZro
...
> **Note:** The EDR check will apply only to machines in the selected groups and will require a running CrowdStrike agent.
...
- Optional. You can experiment and see how the integration works by hiding hosts in the CrowdStrike Host management console:
    ...
    - After about a minute, the peer will be disconnected from the network and marked as `Approval required` in the openZro dashboard.
    - To restore the host in CrowdStrike, navigate to the Trash and click `Restore`

> **Note:** openZro synchronizes the list of devices managed by the EDR platform via the API about every minute.
    The changes might not be visible immediately.

> **Note:** If you install the CrowdStrike agent on a peer after it joined the network, you will need to disconnect and reconnect
