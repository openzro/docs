...
Source: https://docs.netbird.io/manage/access-control/endpoint-detection-and-response/intune-mdm
...
> **Note:** TLDR: Devices marked as "Non-compliant" in Intune will automatically lose access, ensuring strict adherence to your security policies.
...
The integration of openZro with Microsoft Intune provides network security by ensuring only devices managed and compliant
in Intune can access the protected network. This approach ensures only up-to-date and compliant Windows/macOS endpoints have access to critical network resources via openZro and lets administrators enforce access restrictions based on compliance policies defined in Intune, such as device health, OS version, security baseline adherence, and more.

In this guide, you'll learn how to integrate openZro with Microsoft Intune and configure access controls to allow only Intune-managed/compliant devices onto your network.

## Get Started with openZro-Intune Integration

- Navigate to the [Integrations &raquo; EDR](https://your-management.example.com/) tab in the openZro dashboard
...
![openZro Create Application](/docs-static/img/manage/access-control/endpoint-detection-and-response/intune-mdm/create-app.png)
...
## Configure API Permissions for openZro-Intune Integration
...
![openZro Add API Permissions](/docs-static/img/manage/access-control/endpoint-detection-and-response/intune-mdm/api-permissions.png)
...
On the next screen, click on the `Application permissions` button, which will let you select the appropriate permissions for openZro to function correctly with your Microsoft Intune environment.
...
The `DeviceManagementManagedDevices.Read.All` permission allows openZro to read the properties of all devices managed by Microsoft Intune in your organization.
...
## Create a Client Secret for Secure openZro-Intune Authentication
...
![openZro Generate Client Secret](/docs-static/img/manage/access-control/endpoint-detection-and-response/intune-mdm/client-secret.png)
...
![openZro Application ID and Directory](/docs-static/img/manage/access-control/endpoint-detection-and-response/intune-mdm/client-id.png)

## Choose Groups to require Intune Agent

At this stage, specify one or more openZro groups to which the check should apply. The check will require the peer to have a running Intune agent installed.

![Intune Groups](/docs-static/img/manage/access-control/endpoint-detection-and-response/intune-mdm/groups.png)

> **Note:** The MDM check will apply only to machines in the selected groups and will require a running Intune agent.

> **Note:** You can also use groups [synchronized from your Identity Provider (IdP)](/manage/team/idp-sync).
...
- New devices or those that recently achieved compliance may need to be disconnected and reconnected to openZro to propagate updated status.
- openZro regularly synchronizes with Intune every few minutes, so changes in compliance can take some time to reflect on the dashboard.
