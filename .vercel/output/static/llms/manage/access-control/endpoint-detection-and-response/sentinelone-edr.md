...
Source: https://docs.netbird.io/manage/access-control/endpoint-detection-and-response/sentinelone-edr
...
The integration of openZro with SentinelOne provides organizations with robust security controls that allow
...
SentinelOne Singularity, openZro can ensure that only devices with active security monitoring and protection can access the network.

In this guide, we will walk you through the configuration steps to integrate SentinelOne Singularity with openZro and use
...
- Fill in the form:
  ...
  - **Description**: `API token for openZro EDR integration` (optional)
...
> **Note:** Treat the API token securely and store it safely. You will need both the console URL and API token for the openZro integration configuration.

## Configure a SentinelOne Integration in openZro
...
> **Note:** The EDR check will apply only to peers in the selected groups and will require a running SentinelOne agent.
...
> **Note:** openZro matches the SentinelOne agent to the peer using the Serial Number of the device. You must ensure that each of your devices has a unique serial number.
