...
Source: https://docs.netbird.io/manage/activity/event-streaming/amazon-s3
...
You can use Amazon S3 to store and analyze large amounts of data, including network activity events from openZro.

openZro integrates with Amazon S3 and sends activity events to an S3 bucket in real-time once they occur. For every event,
openZro creates a new object in the S3 bucket, which you can then analyze, filter, and query using Amazon tools.

Storing one event per object is not the most efficient way to save data in S3, therefore openZro provides an
...
- Click `Create bucket`
- Give it a descriptive name like `openzro-activity-events`
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:s3:::openzro-activity-events/*"
        }
    ]
}
```
...
- Store `Access key` and `Secret access key` in a secure place. You will need these when configuring an integration in openZro.

## Create an Integration in openZro
- Navigate to the [Integrations &raquo; Event Streaming](https://preview.openzro.io/integrations) tab in the openZro dashboard
...
After configuring the integration in openZro, you can verify that the integration is working correctly by checking
...
The integration test event is sent to validate whether the provided credentials are correct and openZro can stream events.
