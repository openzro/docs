...
Source: https://docs.netbird.io/manage/activity/event-streaming/generic-http

---

# Stream Network Activity to a Generic HTTP Endpoint

The Generic HTTP integration allows you to stream your openZro network activity events to any custom HTTP/S endpoint. This provides a flexible way to connect openZro with a wide range of third-party systems, including custom SIEMs, log management tools like Grafana Loki, or any service that can receive POST requests.

For every event, openZro will send a POST request to your configured endpoint. You have full control over the request's body format and headers, allowing for seamless integration with various APIs.

> **Note:** This feature is only available in the cloud version of openZro.

## Prerequisites

Before you start, ensure you have an HTTP/S endpoint that is publicly accessible and ready to receive POST requests from openZro. You will also need any necessary authentication details, such as an API key or credentials for Basic Authentication.

## Create an Integration in openZro

1.  Navigate to the **Integrations** tab in the openZro dashboard and select **Event Streaming**.
...
-   **Endpoint URL**: Enter the full HTTPS or HTTP URL where openZro should send the events. This field is mandatory.
...
> **Note:** After saving the integration, the configured authentication details will be converted into an HTTP header and will appear in the **Headers** tab.
...
If this option is disabled, openZro sends a default JSON object for each event. When enabled, you can define your own payload structure using Go's `text/template` templating engine.
...
| `.Reference`  | `string`             | A URL to the event in the openZro activity log for easy cross-referencing. | `{"ref": "{{.Reference}}"}`       |
...
Once you save the integration, openZro sends a test event to your endpoint to confirm that the connection and authentication are successful.
...
```json
{
    ...
    "InitiatorID": "openzro-test-system",
    "TargetID": "target-id",
    "Meta": {
        "purpose": "configuration-test",
        "source": "openzro-test"
    },
    "Reference": "openzro:test:generichttp"
}