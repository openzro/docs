...
Source: https://docs.netbird.io/manage/activity/traffic-events-logging

---

# Traffic Events Logging

> **Note:** This feature is available only in the openZro cloud under the [Business plan](https://www.openzro.io/pricing?utm_source=docs&utm_content=traffic-events).
This is feature is in Beta and may change over time — including how data is collected and reported.
To use this feature, make sure you're running openZro client version 0.39 or higher.
...
> **Note:** Use the `P2P` filter in the table to view only peer-to-peer connection events.
...
> **Note:** Use the `Routed` filter in the table to view only peer-to-network resource connection events.
...
> **Note:** For all the examples above, we used the `nc` command to initiate the connection to the CRM server from the peer `Alice`.
...
> **Note:** You can optionally enable `Traffic Reporting (Kernel)` to capture additional details, such as network packet sizes,
...
> **Note:** The blocked event will be available only if the destination uses the openZro client in userspace mode.
...
> **Note:** The UDP connection is stateless, so the stopped event will be generated right after a certain period of inactivity.
...
> **Note:** The blocked event will be available only if the destination is using the openZro client in userspace mode.
 This is the case for Windows and MacOs nodes. For Linux, you can enable userspace mode with the environment variable `NB_WG_KERNEL_DISABLED=true` and `NB_FORCE_USERSPACE_ROUTER=true`
...
> **Note:** The ICMP connection is stateless, so the stopped event will be generated right after a certain period of inactivity.
