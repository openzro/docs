...
> **Note:** **TLDR:**  openZro automatically creates a Default policy that allows all devices to communicate with each other. While this helps with onboarding, the Default policy completely undermines zero-trust principles and should be removed once you create proper access control policies. Make sure to have replacement policies ready before deleting it, or all peer communication will stop.

When you first create a openZro account, a **Default policy** is automatically created that allows all peers to communicate with each other using any protocol. This policy exists because early openZro users were confused and frustrated when their devices couldn't communicate—openZro's deny-by-default security model meant nothing worked without policies defined, and users thought the platform was broken. 

    

The Default policy, which uses the special `All` group as both source and destination, solves this onboarding friction by letting new users immediately see openZro working while they learn the platform.
...
### What Are Groups in openZro?

Groups in openZro function similarly to tags or labels in other platforms. They're collections of peers (devices running the openZro client) and resources (internal network resources that don’t run the openZro client) that can be used in policies to control network traffic. Every peer in your network can belong to one or more groups, and these group memberships determine what resources that peer can access.

### User Groups vs. Peer Groups: A Critical Distinction

This is one of the most important concepts to understand when configuring openZro access control. **It's important to note that openZro doesn't technically differentiate between "user groups" and "peer groups" in the interface—all groups appear together in the Groups page.** However, understanding this conceptual distinction is crucial for implementing secure, well-organized access control.

The distinction is based on **how groups are used and assigned**, not on any technical difference in openZro:
...
- **Manual assignment to users:** Add groups to a user's account in openZro, then any device that user signs into will inherit those groups
...
1. **Manual Creation:** Groups are created explicitly in the openZro interface
2. **Setup Key Assignment:** The most common method - when creating a setup key, you specify which groups should be [auto-assigned](https://docs.openzro.io/manage/peers/register-machines-using-setup-keys#peer-auto-grouping) to any peer that registers with that key
...
Even though openZro treats all groups the same way technically, maintaining this conceptual separation in your mind and in your naming conventions is crucial for several reasons:
...
openZro policies control network traffic by defining:
...
While openZro’s peer-to-peer technology allows servers to connect to laptops and even mobile phones, this is not recommended.
...
Understanding policy directionality is essential for implementing Zero Trust access control. How policies behave depends on whether you're connecting to **peers** (devices running openZro) or **network resources** (internal systems accessed through routing peers).

    

### Policies Between Peers

When both source and destination are peers running the openZro agent, you have full control over directionality:

**Unidirectional:** Traffic flows only from source to destination. In the openZro UI, this appears as a single arrow (→).

Example:

```
Policy: Database Access
Source: app-servers (openZro peers)
Destination: database-servers (openZro peers)
Protocol: TCP
Ports: 5432
Direction: Source → Destination (unidirectional)
```

App servers can initiate connections to database servers on port 5432, but database servers cannot initiate NEW connections back to app servers on that port. Return traffic for established connections is handled automatically by openZro's stateful firewall.

**Bidirectional:** Both groups can initiate connections to each other. In the openZro UI, you can enable this by clicking the bidirectional arrows (⟷).

Example:

```
Policy: Dev Cluster Communication
Source: kubernetes-nodes (openZro peers)
Destination: kubernetes-nodes (openZro peers)
Protocol: ALL
...
**IMPORTANT:** When the destination is a **network resource** accessed through a routing peer (not a peer running openZro directly), policies are **ALWAYS unidirectional** and can ONLY flow from source to destination.

Example:

```
Policy: Access to Private Network
Source: engineering-users (openZro peers)
...
Network resources don't have the openZro agent installed. They don't know the openZro network exists. The routing peer acts as a gateway, forwarding traffic from openZro peers to these resources, but the resources themselves cannot initiate connections back through the openZro network.

Think of it this way:

- ✅ Your laptop (openZro peer) → Routing peer → Office printer (network resource) = **Works**
- ❌ Office printer (network resource) → Routing peer → Your laptop (openZro peer) = **Impossible** (printer doesn't know about openZro)
...
Even though this is unidirectional, return traffic (HTTPS responses) is automatically allowed because openZro implements a stateful firewall that tracks established connections.

### Platform-Specific Implementation

**Linux Platforms:** openZro integrates with native Linux firewall capabilities using connection tracking rules (`ct state established,related accept`). The kernel's connection tracking handles reply traffic automatically.

**Non-Linux Platforms:** openZro implements a stateful firewall in userspace with a connection tracker that dynamically creates temporary rules for reply traffic:
...
Ports: 443, 22
Direction: One-way
Posture Checks: [openZro Latest Version]
...
2. **Keep user groups conceptually separate from peer groups** for clarity and security, even though openZro treats them the same way
