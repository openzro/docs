...
Source: https://docs.netbird.io/manage/networks/use-cases/by-scenario/cloud-to-on-premise
...
```
Cloud VM ────► openZro Tunnel ────► Routing Peer ────► Database Server
(peer)                               (on-prem)         (no openZro)
```
...
Before configuring openZro, identify your data center network's subnet.
...
> **Note:** For more granular access, add specific database IPs instead of the entire subnet. For example, add `10.100.0.50/32` to only allow access to your production database.
...
The routing peer forwards traffic from openZro to your data center network. Install openZro on a server that can reach the database:
...
## Step 6: Deploy openZro on `cloud-workloads`
...
```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sh
sudo openzro up --setup-key YOUR_CLOUD_SETUP_KEY
```
...
Use the [openZro Kubernetes Operator](/manage/integrations/kubernetes) for production deployments.

## Step 7: Verify Cloud Workload Group Assignment

After deploying openZro on your cloud workloads:

1. Go to **Peers** in the openZro dashboard
...
1. Verify the routing peer is online: `openzro status`
