...
Source: https://docs.netbird.io/manage/networks/use-cases/by-scenario/access-home-devices
...
```
Your Laptop ──────► openZro Tunnel ──────► Routing Peer ──────► Home NAS
  (peer)                                    (at home)           (no openZro)
```

## Prerequisites

- A [openZro cloud account](https://your-management.example.com/) or [self-hosted instance](/selfhosted/selfhosted-quickstart)
- openZro installed on your laptop or phone ([installation guide](/get-started/install))
- An always-on device at home to serve as the routing peer (Raspberry Pi, NAS with Docker, old laptop, etc.)

## Step 1: Connect Your Laptop to openZro

If you haven't already, install openZro on your laptop and connect:
...
4. Verify your device appears in the [openZro dashboard](https://your-management.example.com/) under **Peers**
...
Before configuring openZro, identify your home network's subnet.
...
> **Note:** For more granular access, add specific device IPs instead of the entire subnet. For example, add `192.168.1.50/32` to only allow access to your NAS.
...
The routing peer forwards traffic from openZro to your local network. Use any always-on device:
- Raspberry Pi
- Synology NAS
- Apple TV/Android TV
- Home server
...
1. Verify the routing peer is online: check `openzro status` on the routing peer
