...
Source: https://docs.netbird.io/manage/networks/use-cases/by-scenario/remote-worker-access
...
```
Remote Laptop ──────► openZro Tunnel ──────► Routing Peer ──────► Office Server
   (peer)                                    (at office)          (no openZro)
```
...
## Step 1: Connect a Remote Worker Device to openZro

If you haven't already, install openZro on a remote worker's laptop and connect:

1. Download openZro from [your-management.example.com/install](https://your-management.example.com/)
2. Run the application and click **Connect** in the system tray
3. Complete the sign-up process in your browser
4. Verify the device appears in the [openZro dashboard](https://your-management.example.com/) under **Peers**
...
Before configuring openZro, identify your office network's subnet.
...
1. Go to **Networks** in the openZro dashboard
2. Click **Add Network**
...
> **Note:** For more granular access, add specific server IPs instead of the entire subnet. For example, add `10.0.0.50/32` to only allow access to a specific file server.
...
The routing peer forwards traffic from openZro to your office network. Choose an always-on server at your office:
- A dedicated Linux VM
- A Windows Server
- A Docker container on an existing server

**Install openZro on your routing peer:**

1. In the openZro dashboard, go to **Setup Keys**
...
Invoke-WebRequest -Uri "https://github.com/openzro/openzro/releases/latest" -OutFile "install.ps1"; .\install.ps1
...
1. Have employees install openZro from [your-management.example.com/install](https://your-management.example.com/)
