...
Source: https://docs.netbird.io/manage/network-routes/use-cases/by-configuration/advanced-configuration
...
- Source IP is replaced with the routing peer's openZro IP
...
- openZro access control cannot restrict individual source devices
- More complex setup

> **Note:** Without masquerade, traffic from unknown source IPs is rejected by openZro's policy engine. You cannot use ACL Groups or Network Resources without masquerade enabled.
...
> **Note:** The routing peer must be accessible via a policy for the route to activate. The route itself is controlled by policies targeting the ACL Group.
...
Devices without openZro need routes to remote networks through the local routing peer.
...
**Symptoms:** Peers do not see the route; `openzro routes list` shows nothing
...
**Symptoms:** Unexpected traffic going through openZro
...
- openZro distributes traffic across available peers
...
Use openZro's [activity logging](/manage/activity) to monitor access patterns and troubleshoot issues.