...
Source: https://docs.netbird.io/manage/network-routes/use-cases/by-scenario/exit-nodes
...
> **Note:** This feature requires openZro version 0.27.0 or later.
...
> **Note:** IPv6 traffic is currently not supported and is blocked to prevent unintentional leakage.
...
> **Note:** The minimum access policy required for peers to use an exit node is **Users** (source) → **Routing Peer** (destination) over **ICMP**. Ensure a policy with this configuration exists for the distribution groups assigned to the exit node route.
...
> **Note:** The client user's explicit selection or deselection always takes precedence on that device.

> **Note:** Auto Apply requires openZro client version 0.55.0 or later.
...
Navigate to the openZro dashboard and select the **Peers** tab.
