...
Source: https://docs.netbird.io/manage/network-routes/use-cases/by-scenario/site-to-site-home

---

# Site-to-Site: Home Networks

This guide shows how to connect two home networks together so devices can communicate across locations—without installing openZro on every device.
...
```
Home A: NAS ──► Routing Peer ──► openZro Tunnel ──► Routing Peer ──► Parents' NAS
        (no openZro)  (peer)                           (peer)        (no openZro)
```

> **Note:** Site-to-Site requires Network Routes because the Networks feature doesn't yet support bidirectional routing between sites.
...
Before installing openZro on your routing peers, create setup keys with auto-assigned groups:
...
## Step 2: Install openZro on Routing Peers

**At your home:**

```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sh
sudo openzro up --setup-key YOUR_HOME_SETUP_KEY
```
...
```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sh
sudo openzro up --setup-key YOUR_PARENTS_SETUP_KEY
```
...
For devices without openZro to reach the other network, add a static route pointing to the local routing peer.
...
> **Note:** For persistent routes, configure them in your router's DHCP settings or the device's network configuration.
...
1. Verify both routing peers are online and connected: check `openzro status` on each
