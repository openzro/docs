...
Source: https://docs.netbird.io/manage/network-routes/use-cases/by-scenario/site-to-site-office

---

# Site-to-Site: Office Networks

This guide shows how to connect multiple office locations so all devices can communicate—including those without openZro installed.
...
```
HQ Server ──► Routing Peer ──► openZro Tunnel ──► Routing Peer ──► Branch Server
...
Install openZro on a server at each location:

**At headquarters:**

```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sh
sudo openzro up --setup-key YOUR_HQ_SETUP_KEY
```
...
```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sh
sudo openzro up --setup-key YOUR_BRANCH_SETUP_KEY
```
...
2. On the office system that needs to reach remote workers, add a route to the openZro network:

```bash
# Route to openZro network through the routing peer
