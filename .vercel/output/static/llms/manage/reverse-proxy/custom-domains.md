...
Source: https://docs.netbird.io/manage/reverse-proxy/custom-domains

---

# Custom Domains

When you create a reverse proxy service, you need to assign it a domain. openZro provides built-in domains that are automatically available for every account, and also supports custom domains where you bring your own domain name. This page explains how both types work and walks you through adding and verifying a custom domain.

## Built-in domains

Every openZro deployment comes with built-in domains that you can use immediately - no DNS configuration required. The format and labeling of these domains differ between cloud and self-hosted deployments.
...
For example: `myapp.abc123.eu.proxy.openzro.io` where `myapp` is your chosen subdomain, `abc123` is a nonce unique to your account, and `eu` is the proxy cluster region.

openZro operates multiple proxy clusters in different regions, and your available free domains correspond to these clusters. In the domain selector, free domains appear with a **Free** badge.
...
To manage custom domains, navigate to **Reverse Proxy** > **Custom Domains** in the openZro dashboard.

## Adding a custom domain

Follow these steps to add a custom domain to your account:

1. Navigate to **Reverse Proxy** > **Custom Domains** in the openZro dashboard.
2. Click **Add Domain**.
...
To prove that you own the domain, openZro requires you to create a specific CNAME record in your DNS provider.
...
- **Cloud deployments** - point to a openZro-hosted cluster address (e.g., `eu.proxy.openzro.io`)
...
|-------------|------|-------|
| `CNAME` | `*.proxy.example.com` | `eu.proxy.openzro.io` |
...
The exact target value depends on the proxy cluster you selected when adding the domain. The openZro dashboard displays the required CNAME record and target after you save the domain.
...
openZro performs a CNAME lookup on `*.<your-domain>` and verifies that it resolves to a known proxy cluster. Once verification succeeds, the domain status changes to **Active** and it becomes available in the domain selector when creating or editing services.

> **Note:** DNS changes can take time to propagate. If openZro does not find the record immediately, please wait up to 24 hours and try again.
...
> **Note:** Before deleting a custom domain, make sure no active services are using it. Services referencing a deleted domain will no longer route traffic correctly.
...
Verify that the wildcard CNAME record for `*.<your-domain>` is correctly configured and points to the right proxy cluster address. For cloud deployments, this is a openZro-hosted address (e.g., `eu.proxy.openzro.io`). For self-hosted deployments, this is your own proxy URL (e.g., `proxy.mycompany.com`). Use a DNS lookup tool to confirm the record has propagated:
...
Each custom domain must be unique across all openZro accounts. If you receive an error that the domain is already in use, it may be registered by another account. Verify that you are entering the correct domain name. If you believe this is an error, contact [openZro support](/slack-url).
