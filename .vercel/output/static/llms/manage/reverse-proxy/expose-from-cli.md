...
Before using `openzro expose`, make sure:

- The openZro client is connected — run `openzro up` first.
...
An account administrator must enable peer expose before any peer can use the `openzro expose` command.

### From the dashboard

1. Navigate to **Settings** > **Clients** in the openZro dashboard.
...
```bash
curl -X PUT "https://api.openzro.io/api/accounts/{account_id}" \
...
```shell
openzro expose --protocol tcp 5432
```
...
```shell
openzro expose --protocol tcp --with-external-port 15432 5432
```

If the proxy cluster does not support custom ports (e.g., openZro shared cloud clusters), the port is auto-assigned. When you explicitly specify `--with-external-port` but the cluster overrides your choice, the output indicates the reassignment:
...
> **Note:** Authentication flags (`--with-pin`, `--with-password`, `--with-user-groups`) are only supported for HTTP/HTTPS protocols. L4 protocols (TCP, UDP, TLS) do not support these flags because there is no HTTP layer to serve a login page. Similarly, `--with-external-port` is only supported for L4 protocols.
...
```shell
openzro expose 8080 --with-pin 123456
```
...
```shell
openzro expose 8080 --with-password mysecretpassword
```
...
```shell
openzro expose 8080 --with-pin 123456 --with-password backup-password --with-user-groups admin
```

> **Note:** If no authentication flag is provided, the service is publicly accessible without any protection. Consider always using at least one authentication method for sensitive services.
...
By default, `openzro expose` generates a random subdomain on your proxy cluster domain (e.g., `a1b2c3d4e5f6.proxy.example.com`). You can customize this in two ways.
...
```shell
openzro expose 8080 --with-name-prefix myapp
```
...
> **Note:** The custom domain must already be configured and verified in your account before it can be used with the expose command. See [Custom Domains](/manage/reverse-proxy/custom-domains) for setup instructions.
...
When you run `openzro expose`, the following happens:

1. The CLI sends the request to the local openZro daemon.
...
> **Note:** The 90-second TTL means the service may remain accessible for up to 90 seconds after the client disconnects unexpectedly. During this window, the domain still resolves and routes traffic. For sensitive services, always use authentication to prevent unauthorized access.
...
The `openzro expose` command requires permission at two levels:
...
|----------|-------------------|---------------------|
| **Created via** | Dashboard or API | `openzro expose` command |
...
### "client is not running, run 'openzro up' first"

The openZro daemon is not connected. Start it with:
...
openzro expose 3000 --with-name-prefix demo
```
...
```shell
openzro expose 8080 --with-password staging-review --with-name-prefix staging
```
...
```shell
openzro expose 8080 --with-user-groups engineering --with-name-prefix internal-api
```
...
```shell
openzro expose 9000 --with-name-prefix webhooks
```
...
```shell
openzro expose --protocol udp --with-external-port 27015 27015
```
...
```shell
openzro expose --protocol tls --with-custom-domain tls.example.com 4443
```

## Related pages

- [Reverse Proxy](/manage/reverse-proxy) - overview of the reverse proxy feature, dashboard setup, and concepts
- [Authentication](/manage/reverse-proxy/authentication) - SSO, password, PIN, and header authentication, plus access restrictions
...
- [CLI Reference](/get-started/cli#expose) - `openzro expose` command reference