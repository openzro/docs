...
Source: https://docs.netbird.io/manage/reverse-proxy/troubleshooting
...
This applies to self-hosted setups where an external identity provider (such as Authentik or PocketID) is exposed through the openZro reverse proxy. For example, the architecture described in [Self-host openZro with Authentik](https://openzro.io/knowledge-hub/selfhost-openzro-with-authentik). The management server (Dex) needs to reach the IdP's OIDC discovery endpoint through the reverse proxy to initialize the connector. However, traffic originating from inside the Docker network does not carry a valid geo-IP. The proxy sees an internal Docker IP instead of a public IP, which doesn't match any country, so the restriction rejects the request with a 403.

> **Note:** This issue only applies to self-hosted deployments where the IdP is exposed through the openZro reverse proxy and a restriction (such as geo-IP country filtering or access controls) is added to that service. If your IdP is hosted externally, accessible via a separate domain, or the reverse proxy service has no restrictions configured, this does not apply.
...
```
docker run --rm --network container:openzro-server curlimages/curl -s https://<your-idp-domain>/.well-known/openid-configuration
```
...
```
docker run --rm --user root -v <openzro_data_volume>:/data --entrypoint sqlite3 keinos/sqlite3 /data/store.db \
...
|---|---|
| **Peer** | The service runs directly on a machine that has the openZro client installed. |
| **Host / Subnet** | The service runs on a device that does not have a openZro client and relies on a routing peer to forward traffic to it on the local network. |
