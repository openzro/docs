...
Source: https://docs.netbird.io/manage/peers/remote-jobs
...
> **Note:** Remote Jobs require the target peer to be **online** and running **openZro v0.64.0 or later**.
...
On success, the job returns an **upload key** that can be used to retrieve the debug bundle. Share this key with openZro support via GitHub Issues, Slack, or your support channel.
...
> **Note:** The **Run Remote Job** button is disabled when the peer is offline or when you lack the required permissions.
...
```bash
curl -X POST https://api.openzro.io/api/peers/{peerId}/jobs \
...
```bash
curl https://api.openzro.io/api/peers/{peerId}/jobs \
  -H "Authorization: Token <PAT>"
```
...
```bash
curl https://api.openzro.io/api/peers/{peerId}/jobs/{jobId} \
  -H "Authorization: Token <PAT>"
```
...
- Peers must be running **openZro v0.64.0 or later**
