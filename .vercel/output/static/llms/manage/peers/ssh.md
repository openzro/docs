...
Source: https://docs.netbird.io/manage/peers/ssh
...
> **Note:** SSH Access is disabled by default and must be enabled both on the target
    peer and in the openZro Dashboard.
...
1. **openZro v0.61.0 or later** on all peers using SSH (both clients and servers). For fine-grained access control features, v0.61.0 is required.
2. **SSH enabled on target peer** - the peer must run with `--allow-server-ssh`.
3. **Access Control Policy** - A `openZro SSH` policy configured in the dashboard
4. **(Optional)** For JWT authentication: an Identity Provider configured in openZro.

> **Note:** Management distributes openZro access-policy objects (FirewallRule) - it does
    not directly open host OS ports or edit host-level firewalls. Enforcement and
    forwarding are handled by the openZro client running on each peer.
...
1. Click on the openZro tray icon
...
> **Note:** Currently, only OpenSSH clients are supported. Other SSH clients (PuTTY,
    etc.) are not compatible.

**Prerequisites:**

openZro creates `/etc/ssh/ssh_config.d/99-openzro.conf` (or equivalent on your OS) which automatically:
1. Detects if the target is a openZro SSH server
...
1. OpenSSH reads the openZro configuration from `99-openzro.conf`
2. openZro detects if the target is a openZro SSH server
3. If detected, openZro handles JWT authentication automatically (OIDC flow if needed)
...
openZro supports fine-grained user mapping, allowing you to specify which OIDC users or groups can connect to which local OS users on the target system. See [Fine-Grained Access Control](#fine-grained-access-control) for details.

> **Note:** **Time synchronization required**: The SSH server's system clock must be
    synchronized with the IdP for JWT validation. Clock skew can cause
    authentication failures.

**How it works:**

1. When you connect via SSH, the openZro client requests a JWT token
...
> **Note:** When JWT authentication is disabled, SSH access is controlled solely by
...
1. **Source User Groups**: Which openZro user groups are allowed to connect
...
2. Select **openZro SSH** as the protocol
...
> **Note:** When **Full Access** is selected, users in the source groups can SSH as any local user on the destination machines. Use **Limited Access** to enforce the principle of least privilege.
...
3. openZro checks if the user's ID is in the list of authorized users for this peer
4. openZro verifies the user is allowed to access the requested local OS user based on the policy's authorized groups configuration
5. If authorized, the SSH session is established as the specified local user

> **Note:** Fine-grained access control requires JWT authentication to be enabled (the default). It is not available when using `--disable-ssh-auth`.
...
> **Note:** Port forwarding must be enabled on the SSH server using
...
> **Note:** SFTP must be enabled on the SSH server using the `--enable-ssh-sftp` flag.
...
openZro now supports fine-grained user mapping through the `openZro SSH` protocol and Authorized Groups configuration. See [Fine-Grained Access Control](#fine-grained-access-control) for details.
...
1. **Network ACL policies**: Control which peers can access SSH with dedicated 'openZro SSH' protocol policies
2. **JWT authentication** (if enabled): User identity verification via IdP
3. **Fine-grained access control** (when using openZro SSH protocol): Controls which authenticated users can access which local OS users on the target machine
...
> **Note:** **Breaking change**: Management server v0.61.0 will break openZro SSH for all peers running versions older than v0.61.0. You must update all peers that use SSH to v0.61.0 or later before upgrading the management server/dashboard.
...
> **Note:** This section is only relevant if you're upgrading from openZro v0.59.x or earlier.
...
> **Note:** **Update order is important** - SSH servers must be updated before SSH clients.
...
5. **Update SSH clients last** (peers using `openzro ssh`)
