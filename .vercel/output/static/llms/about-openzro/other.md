...
Source: https://docs.netbird.io/about-openzro/other
...
Open a [GitHub Discussion](https://github.com/openzro/openzro/discussions) to talk to potential mentors.

### Task

The goal of the task is to get familiar with the system by setting up a self-hosted version of openZro.

1. Carefully review projects' documentation.
2. Deploy a self-hosted version of openZro.
3. Register at least 2 peers in the system (can be a laptop, server or even Raspberry PI).
> **Note:** It is possible to set up multiple peers on the same machine. Find out how!
...
You could skip #1 and #2 starting with #3 using managed version of openZro (https://your-management.example.com).

### Project Ideas

There are a few categories that our ideas fall into:
* **UI** - mostly around our Management service Dashboard ([Github repo](https://github.com/openzro/dashboard))
* **Client** - our client application ([Github Repo](https://github.com/openzro/openzro/tree/main/client))
* **Server** - our Management and Signal server applications ([Management Github Repo](https://github.com/openzro/openzro/tree/main/management) and [Signal Github Repo](https://github.com/openzro/openzro/tree/main/signal))
...
### Idea: Make openZro run on routers
> **Note:** OpenWrt is an open-source project for embedded operating systems based on Linux, primarily used on embedded devices to route network traffic.

https://openwrt.org/

openZro runs on different platforms and can also be compiled to run on routers. For that, we are thinking to work on this support for the popular wireless router OpenWRT.
...
> **Note:** eBPF is a revolutionary technology with origins in the Linux kernel that can run sandboxed programs in an operating system kernel.

https://ebpf.io/

When running on machines openZro client application tries to be as efficient as possible.
On Linux machines openZro can use kernel modules like Wireguard and features like eBPF to be the most efficient.

Besides connectivity, openZro can control access. Right now it is achieved on a connection level -
...
### Idea: openZro client running in the browser

> **Note:** WebAssembly (abbreviated Wasm) is a binary instruction format for a stack-based virtual machine.
...
openZro requires a client application installed to connect machines to each other.
...
The idea of this project is to make openZro run in the browser using WASM and WebRTC.
...
>
> **Goal:**  Interactive console running in the browser establishing connection to the openZro network.
>
...
### Idea: Integrate openZro with Kubernetes

> **Note:** Kubernetes, also known as K8s, is an open-source system for automating deployment, scaling, and management of containerized applications.
...
The idea of this project is to integrate openZro with Kubernetes by creating a controller that would add an agent
...
>
> **Goal:**  Integrate openZro with Kubernetes for automatic provision of agents.
>
...
### Idea: openZro Github Action

> **Note:** GitHub Actions makes it easy to automate all your software workflows, now with world-class CI/CD. Build, test, and deploy your code right from GitHub.
...
The idea of this project is to create a openZro agent action, that would download and run an ephemeral agent on Github Actions workflows. The benefits of this
...
>
> **Goal:**  Create a openZro Github Action.
>
...
>
> **Reference:** https://docs.openzro.io/getting-started/quickstart
>
...
The idea of this project is to create a openZro mobile app for either Android or iOS that would follow one of the principles of our product, simplicity.
...
>
> **Goal:**  Create a openZro Mobile Client App.
>
