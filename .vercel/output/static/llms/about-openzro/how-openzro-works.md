...
Source: https://docs.netbird.io/about-openzro/how-openzro-works

---

# How openZro Works

openZro is an open source platform consisting of a collection of components, responsible for handling peer-to-peer
...
and [software](https://github.com/openzro/openzro) developed by openZro to make secure private networks deployment and management simple.

openZro relies on four components - **Client** application (or agent), **Management**, **Signal** and **Relay** services.
...
With openZro clients installed and authorized on the Management service, machines form a mesh network connecting
...
Let's now take a closer look at each of openZro's components.

## Management Service

The Management service is the central coordination component for openZro with a UI dashboard.
...
After installation, openZro client requires login that can be done through Identity Provider (IDP) like Okta or with a [setup key](/manage/peers/register-machines-using-setup-keys).
...
The Management service runs in the cloud openZro-managed. It can also be self-hosted.

    

## Client Application

The openZro Client application (or agent) is a software that is installed on the machines within a openZro network.
...
* **Applying DNS settings.** The client application runs an [embedded DNS resolver](https://openzro.io/knowledge-hub/using-xdp-ebpf-to-share-default-dns-port-between-resolvers) and ensures that the machine can resolve other peers' FQDNs in the `openzro.cloud` namespaces.
The client also applies other DNS settings that are received from the Management service.

> **Note:** The **private key**, generated by the openZro client, **never leaves the machine**, ensuring that only the machine that owns the key can decrypt traffic addressed to it.
...
> **Note:** Messages that are sent over Signal are **point-to-point encrypted**, so Signal can't see the contents.

openZro Signal is very similar to the signaling servers used in [WebRTC](https://developer.mozilla.org/en-US/Web/API/WebRTC_API/Signaling_and_video_calling#the_signaling_server).
It runs in the cloud openZro-managed and can be self-hosted.
