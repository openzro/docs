...
Source: https://docs.netbird.io/about-openzro/browser-client-architecture

---

# Browser Client Technical Architecture

The openZro Browser Client enables secure remote connections directly from web browsers without requiring client software installation. This document provides a technical overview of the Browser Client's architecture, implementation, and security model.

## Architecture Overview

The Browser Client architecture implements a complete openZro peer in the browser environment.
...
The Browser Client runs two WASM modules: the openZro client (which handles SSH, WireGuard tunneling, and networking) and IronRDP (for RDP protocol handling).

#### 1. WebAssembly openZro Client

The heart of the Browser Client is a openZro peer compiled to WebAssembly (WASM):
...
The WASM client shares the same codebase as native openZro clients but is adapted for the browser environment. All traffic routes through openZro relay servers using WebSocket while maintaining end-to-end WireGuard encryption.
...
IronRDP communicates with target RDP servers through a custom Go-based RDCleanPath bridge that runs within the openZro WASM client, routing traffic through openZro tunnels.

#### 3. Protocol Bridges

##### SSH Bridge

SSH connections are handled by the openZro WASM client with a browser-based terminal UI:

- **SSH Client**: Implemented by the openZro client
- **Terminal UI**: [xterm.js](https://github.com/xtermjs/xterm.js) provides the terminal interface
- **Connection**: The SSH client connects to openZro's embedded SSH server (port 44338) on target peers
...
- **Server-side**: A custom Go implementation of the RDCleanPath server runs within the openZro WASM client
- **Function**: Bridges RDP traffic from the RDCleanPath proxy through openZro tunnels to the target RDP server
- **Security**: Handles certificate validation and caching

### Management Server
To support the Browser Client functionality, the openZro management server uses two resources:

#### Temporary Peer
The WebAssembly openZro Client will register as a temporary peer with the management server. The temporary peer will live as long as the connection is active.
...
For the WebAssembly openZro Client all peers will be named as `{browser}-browser-client` (e.g. `safari-17-browser-client`).

    

#### Temporary Policy
To allow the registered WebAssembly openZro Client to connect to the target peer, a temporary policy will be created. The temporary policy will allow access to the target peer on ports 22 (SSH), 3389 (RDP).
...
The policies for the WebAssembly openZro Client will be named as `Temporary access policy for peer {browser-client-name}` (e.g. `Temporary access policy for peer safari-17-browser-client`).
...
The Browser Client uses openZro's temporary access mechanism:
...
```
User Input → Browser Events → [WASM: Protocol Client (SSH/IronRDP) → openZro Tunnel] →
...
The Browser Client communicates with openZro services through WebSocket proxies:
...
These proxies run alongside the respective openZro services and translate between browser-compatible WebSocket and internal gRPC protocols.