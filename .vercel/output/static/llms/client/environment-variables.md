...
Source: https://docs.netbird.io/client/environment-variables

---

# Client Environment Variables

The openZro client supports environment variables for advanced configuration. These are separate from the CLI flag environment variables (like `NB_LOG_LEVEL`) which are described in the [CLI reference](/get-started/cli#environment-variables).

## Setting environment variables

Environment variables must be set on the **daemon process**, not the CLI. When running openZro as a system service, use `--service-env` to persist them:

```shell
# Set during install
sudo openzro service install --service-env NB_WG_KERNEL_DISABLED=true

# Or reconfigure a running service
sudo openzro service reconfigure --service-env NB_WG_KERNEL_DISABLED=true

# Multiple variables
sudo openzro service install --service-env NB_LOG_LEVEL=debug,NB_USE_LEGACY_ROUTING=true
```

When running in foreground mode, set them in the shell before starting:
```shell
NB_WG_KERNEL_DISABLED=true openzro up --foreground-mode
```

> **Note:** To clear all saved service parameters (including env vars), run `sudo openzro service reset-params` followed by `sudo openzro service reconfigure`.
...
| `NB_NFTABLES_TABLE` | Linux | Name of the nftables table the client creates for ACL rules (default: `openzro`). Change this if the default name conflicts with existing firewall configuration. |
...
| `NB_UNCLEAN_SHUTDOWN_RESOLV_FILE` | Linux, FreeBSD | Custom file path to store a backup of `/etc/resolv.conf` before the client modifies it. Used to restore the original on next startup if the client crashed without cleaning up (default: `<state-dir>/resolv.conf`, i.e. `/var/lib/openzro/resolv.conf` on Linux, `/var/db/openzro/resolv.conf` on FreeBSD). |
...
| Variable | Platform | Description |
|---|---|---|
| `NB_DISABLE_SSH_CONFIG` | All | Prevent the client from writing openZro peer entries into the system SSH config directory (`/etc/ssh/ssh_config.d/99-openzro.conf` on Unix, `%PROGRAMDATA%\ssh\ssh_config.d\99-openzro.conf` on Windows). By default, the client adds `Host` blocks so you can `ssh peer-name` without specifying the IP. |
...
| Variable | Platform | Description |
|---|---|---|
| `NB_STATE_DIR` | All | Override the directory where the client stores persistent state files (config, state.json, WireGuard keys). Defaults to `/var/lib/openzro` on Linux/macOS, `/var/db/openzro` on FreeBSD, `%PROGRAMDATA%\openZro` on Windows. |
| `NB_DNS_STATE_FILE` | All | Override the path to the client state file (`state.json`). Despite the name, this is not DNS-specific: it stores all shutdown/recovery state including DNS settings, SSH config, routes, and auto-update progress. The default path is per-profile (e.g. `/var/lib/openzro/state.json` for the default profile). Setting this bypasses per-profile state isolation. Primarily used by the embedded client API (`embed.Options.StatePath`). |
