...
### 0.1 Required openZro plan and features
...
### 0.2 Firewall rules for openZro traffic

openZro peers do not need inbound ports open. They initiate outbound connections to openZro's Management, Signal, and Relay services (as well as other peers) over HTTPS (TCP port 443) and UDP/STUN/TURN protocols. If your network firewall restricts outbound traffic (egress filtering), you must allow these specific endpoints and ports.

> **Note:** For a complete list of openZro endpoints, IP addresses, and detailed firewall configuration examples, see the [FAQ: What firewall ports should I open to use openZro?](/about-openzro/faq#what-firewall-ports-should-i-open-to-use-openzro-network-firewalls)
...
> **Note:** - For a list of outbound ports required for openZro Cloud, see the [FAQ: What firewall ports should I open to use openZro?](/about-openzro/faq#what-firewall-ports-should-i-open-to-use-openzro-network-firewalls)
- For a list of outbound ports required for self-hosted openZro, see the [self-hosted guide](/selfhosted/selfhosted-guide#port-requirements)

---

## 1. Terminology and naming

This section defines how terms are used in this guide and in openZro.

> **Note:** **On capitalization:**

**[Network](/manage/networks)** with a capital N refers to the openZro object that represents one or more CIDR ranges and DNS resources behind one or more routing peers.

**network** (lowercase) refers to generic networking concepts such as LANs, VPCs, or the internet.

### 1.1 Core openZro terms

- **Peer**

    A device running the openZro client. Each peer has an overlay IP address and can belong to one or more groups.

- **Routing peer**

    A openZro peer that acts as a gateway, forwarding traffic between the openZro overlay network and one or more internal subnets (private network ranges). Routing peers typically run in a VPC (Virtual Private Cloud, such as AWS VPC or Azure VNet), data center, or office LAN (Local Area Network). Think of it as a bridge between openZro's virtual network and your physical or internal networks.

- **Overlay network and overlay IP**

    The overlay network is a virtual network that openZro creates on top of your existing infrastructure (a "network on top of a network"). For each openZro tenant, the overlay is a random /16 CIDR block within the `100.64.0.0/10` range (a private IP range reserved for carrier-grade NAT). Each peer receives an overlay IP address from this /16 range for communication with other peers. Internal networks never see WireGuard encryption keys or tunnels directly. They only see overlay IPs or the routing peer's IP address, depending on whether masquerading is enabled (see Section 6.2).

- **Network (openZro object)**

    A openZro configuration object that maps your internal networks (such as VPCs, LANs, or office networks) and organizes routing peers and resources. [Networks](/manage/networks) group routing peers that provide access to the same internal subnets. Resources within Networks (IP addresses, ranges, or DNS names) serve as destinations in access policies. Networks replaced the older "Network Routes" concept starting with openZro v0.35.0. Resources within a Network must be assigned to groups, and access is controlled through policies. Resources only become visible to peers after a policy explicitly grants access.
...
- **SIEM (Security Information and Event Management)**
    ...
    A security platform that collects, analyzes, and stores security events from various sources. Examples include Datadog, AWS Firehose, and S3-based pipelines. SIEMs ingest openZro events (traffic flows, policy changes, authentication events) for visibility, alerting, and compliance reporting.
...
## 2. Planning phases before you touch openZro

These three phases are the planning work you do before and while you start configuring openZro. They keep the scope small and make later implementation steps predictable.
...
- Later, openZro Traffic Events and SIEM once peers are enrolled

Most admins do not have perfect flow logs. Treat this as a rough first pass that you will refine using traffic data once openZro is in place. If you lack flow data, interview application owners or review firewall deny logs to fill in the gaps.
...
When you implement this in openZro, you can start slightly more permissive for a short period and use Traffic Events and SIEM to validate your assumptions, then tighten down to the final allow list.
...
The next sections show how to turn these phases into concrete openZro configuration:
...
Connect openZro to your IdP (Identity Provider, for example Microsoft Entra ID, Google Workspace, Okta) and enable SSO (Single Sign-On) with MFA (Multi-Factor Authentication). This allows users to log in to openZro using their existing corporate credentials instead of separate openZro accounts.

Where to do this: [Integrations → Identity Provider Sync](/manage/team/idp-sync).

Map:

- Your IdP groups (for example `DevOps`, `Support`, `Finance`) to openZro groups where it makes sense
...
> **Note:** For the full traffic discovery workflow that you can reuse per scope, see [Section 7.2](#72-use-the-siem-for-discovery-and-verification), which covers flow discovery using event logs. Enable Traffic Events and streaming now so you can use that workflow as soon as peers are enrolled.
...
- `net-<location>-` for routed Networks
...
- Routing peers for openZro Networks
...
- Your SIEM receives openZro events and you can filter by peer, user, and group
...
openZro is deny-by-default. Without policies, peers cannot talk to each other.

To avoid early confusion, openZro creates a Default policy in new environments that allows every peer to communicate with every other peer on all protocols and ports. This is useful only as a temporary onboarding aid.
...
> **Note:** Before deleting the Default Policy, ensure you have created and tested at least one specific Allow rule. Otherwise, you will immediately lose connectivity to all peers.

### 4.2 Posture checks and their limits

openZro posture checks evaluate device state before allowing traffic. They can validate:
...
> **Note:** Posture checks are not a security boundary against compromised endpoints. They validate configuration and policy compliance, not trustworthiness. An attacker with admin rights on a device can bypass or spoof them. Design your policies assuming posture checks can be evaded.

Recommended posture checks per policy:

- **Client version**
    - Enforce minimum openZro version (for example `>= 0.45.0`) for critical access.
- **OS and version**
...
> **Note:** The **Peer Network Range (CIDR)** posture check is not supported on iOS and Android. If you need location-based restrictions for mobile clients, consider using the **Geolocation** posture check as an alternative.
...
- **Linux production servers**
    ...
    - openZro client at or above your standard version.
- **Admin laptops**
...
> **Note:** The example above uses group names for readability. The actual API requires group IDs (UUIDs), not names. You can retrieve group IDs via the [Groups API endpoint](/api/resources/groups#list-all-groups).
...
Broken DNS is the most common cause of "openZro is broken" complaints. For every environment where peers need internal resources or routed Networks, explicitly allow DNS traffic to the correct servers.
...
openZro Network access policies are unidirectional. They allow traffic from openZro peers toward the routing peer and the routed network. Replies to that traffic work as expected, but new connections cannot originate from the private network toward a openZro peer through the routing peer.

If you need true bidirectional initiation between two endpoints, run openZro on both machines and create an access policy that allows traffic in both directions.

> **Note:** If you need to access the routing peer itself (for example, to reach services like Pi-hole or a monitoring dashboard running on it), create a peer-to-peer access policy with the routing peer's group as the destination. See [Routing Peers](/manage/networks#routing-peers) for details.
...
With Networks and domain resources, openZro can route both IP ranges (like `10.10.0.0/16`) and DNS-based resources (like `*.corp.internal`), and can forward DNS lookups through the routing peer.
...
- Add a DNS server in openZro with the right match domain (for example `corp.internal`) under [DNS → Nameservers](/manage/dns/internal-dns-servers#configuring-nameservers-for-internal-domains).
- Add explicit policies allowing peers to reach the DNS servers themselves (UDP and TCP 53 to the resolver IPs).

> **Note:** Starting with openZro v0.59.0, the local DNS forwarder port changed from 5353 to 22054. The Management Service only applies the new port when all peers in the account run v0.59.0 or newer. If you have mixed client versions, DNS forwarding may behave differently across peers.

> **Note:** **Verification checklist:**
- Peers can connect to internal IPs behind the routing peer
- DNS names hosted inside those networks resolve correctly on openZro peers
...
- Monitor routing peers with your existing monitoring plus openZro events. If one fails, verify that clients start using the other peers.
- Verify HA behavior by simulating a failure and checking route selection with `openzro status -d` and test traffic.
...
Masquerading (also called NAT masquerading or source NAT) controls how routed traffic appears to internal networks. When enabled, openZro rewrites the source IP address of traffic so it appears to come from the routing peer's internal IP address instead of the original openZro overlay IP.
...
This usually works out of the box because internal firewalls already trust the routing peer's subnet. openZro enables masquerading by default.

**With masquerading disabled**, internal systems see the original openZro overlay IP:

- Internal firewall log:
    - `100.64.1.45 → 10.10.1.10:5432`
    - where `100.64.1.45` is a openZro overlay address (always within `100.64.0.0/10`)
...
- **AWS VPC route table:**
    - Destination: `100.64.0.0/10` (the openZro overlay range)
...
If internal systems initiate traffic toward openZro peers (health checks, callbacks), you must:
...
openZro has route selection tools and an experimental lazy connections feature for large networks. In practice:
...
- [Traffic Events](/manage/activity/traffic-events-logging) in the openZro dashboard
...
openZro's [Control Center](/manage/control-center) provides a topological view of who can reach what, based on policies, groups, and Networks. Access control policies can be edited directly from the Control Center.
...
- Maintain one "break glass" access method (for example, a physical console or a separate VPN/bastion) that does *not* rely on openZro during the initial setup phase.
...
- Keep an eye on:
    - openZro client versions (enforce minimum via posture checks)
...
- Posture check failing:
    - Device OS or openZro version does not meet the policy requirement.
...
- Temporarily deselect problematic routes and see if the issue disappears.
    ...
    ```bash
    # For Networks
    openzro networks deselect "network_name"
    openzro networks select "network_name"

    # For legacy routes
    openzro routes deselect "route_id"
    openzro routes select "route_id"
    ```
...
- No replacement policies exist yet, and openZro's deny-by-default behavior is kicking in.
...
- **Linux:** `resolvectl status` (systemd-resolved command to see if the openZro interface has the right DNS domains configured)
...
Where to configure: **Settings → Authentication** in the openZro dashboard.
...
- **User approval**
    ...
    Require administrator approval for new user accounts before they can access openZro. This ensures only authorized personnel can enroll devices and access resources.
...
- **Global MFA (if your IdP does not enforce it)**
    ...
    Enable Multi-Factor Authentication at the openZro level if your Identity Provider does not enforce MFA. This adds a second authentication factor to protect against credential theft and unauthorized access.

### 10.2 Permission settings

Configure these settings to restrict dashboard access.

Where to configure: **Settings → Permissions** in the openZro dashboard.
...
- At least two routing peers per critical openZro Network.
- Different failure domains (AZs, racks, hosts).
- Primary metric 100, backup metric 200.
- Monitor both openZro health and OS-level metrics.