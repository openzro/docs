# openZro client on MikroTik router

Source: https://docs.netbird.io/use-cases/homelab/client-on-mikrotik-router
...
In this guide we'll deploy openZro client in a MikroTik container.

## Use cases
Running openZro on MikroTik router or CHR enables cost-effective remote access to RouterOS devices (and their networks)
...
### Branch offices
Not all remote locations have a server room or a similar setup where they can deploy an additional machine to run openZro, i.e. small shops or branch offices that barely have a network cabinet to fit a switch and a router.

Running openZro directly on a router allows us to have remote access to perform basic network management and monitoring without having to maintain an additional machine for openZro router, or even worse—using one of the business-critical openZro clients as a router.
...
Team members would still run openZro on computers and phones, but a separate IT or infra team needs to be able to remotely manage MikroTik devices to help with unpredicted issues in the field. E.g., reconfigure the router to piggyback the entire network over the location's guest Wi-Fi, or quickly switch between Wi-Fi, backup cellular or satellite, depending on the type of failure.

Traditionally, we would always have cellular modems in routers for minimal management connectivity and then run CHR in a cloud VM. Those routers would all start VPN tunnels to the cloud VM so the IT team can connect to the router if needed. On top of that, we would need an additional openZro router in the cloud to enable remote access from openZro to that cloud router and NAT to remote devices.

Running openZro directly on field routers removes the need for a lot of complexity because there's no longer a need for CHR to serve as a VPN concentrator or a dedicated VM to route openZro clients to MikroTiks.

## Limitations
> **Note:** Use at your own risk. All [RouterOS containers warnings](https://help.MikroTik.com/docs/display/ROS/Container#Container-Disclaimer) apply.\
  This is unsupported by both MikroTik and openZro because it uses MikroTik's beta and openZro's legacy features.
...
standard Kubernetes or Docker platforms, so openZro can't take advantage of kernel modules or netfilter rules.\
...
- openZro in RouterOS containers can not use an exit node (because it uses legacy routing mode).
- openZro in RouterOS containers can't perform NAT, but it can do direct routing, and we can do NAT on RouterOS instead.
...
Create a bridge interface for containers and VETH interface for openZro container:
```shell
/interface/veth/add name=openzro address=172.17.0.2/24 gateway=172.17.0.1
/interface/bridge/add name=containers
/ip/address/add address=172.17.0.1/24 interface=containers
/interface/bridge/port add bridge=containers interface=openzro
```
Set up NAT for containers so they can access the internet and other networks:
```shell
/ip/firewall/nat/add chain=srcnat action=masquerade src-address=172.17.0.0/24
```
Because openZro in RouterOS containers can't perform NAT, we'll want to add a route from MikroTik to our openZro subnet via openZro container.
This assumes our openZro subnet is `100.80.0.0/16`.
```shell
...
### Prepare the openZro container
```shell
/container/mounts/add name=openzro_etc src=disk1/etc dst=/etc/openzro
```
Note that we placed `/etc/openzro` on router's built-in flash. This is because we don't want someone stealing the USB drive
and getting access to router's private keys. This file doesn't really change all that often so it's ok to put it there.

```shell
/container envs
add key=NB_SETUP_KEY name=openzro value=YOUR_NETBIRD_SETUP_KEY
add key=NB_NAME name=openzro value=CONTAINER_HOSTNAME
add key=NB_HOSTNAME name=openzro value=CONTAINER_HOSTNAME
add key=NB_LOG_LEVEL name=openzro value=info
add key=NB_DISABLE_CUSTOM_ROUTING name=openzro value=true
add key=NB_USE_LEGACY_ROUTING name=openzro value=true
```
...
If using a self-hosted openZro server we'll also want to add the correct URLs to our server:
```shell
add key=NB_MANAGEMENT_URL name=openzro value=YOUR_NETBIRD_MANAGEMENT_URL
add key=NB_ADMIN_URL name=openzro value=YOUR_NETBIRD_ADMIN_URL
```

Create the container and trigger image pull from DockerHub:
```shell
/container/add remote-image=openzro/openzro interface=openzro root-dir=/usb1/openzro_filesystem mounts=openzro_etc envlist=openzro dns=10.71.71.1 hostname=openzro logging=yes                                                                 │
```
...
At this point we should see the container in our openZro dashboard and we should be able to create routes through it in openZro.
Via the openZro dashboard, create a route through the container to the router's bridge IP address.
Address will be 172.17.0.1/32 and routing peer will be our container. Don't forget to disable NAT on this route.

## Troubleshooting
1. Increase openZro's verbosity by setting `NB_LOG_LEVEL` env var to `trace`.\
...
Assuming that our container keeps stopping because openZro is crashing, we can override the container entrypoint
...
```
When done revert the entrypoint back to openZro:
```shell
/container/set entrypoint="" numbers=0
```

### openZro starts and logs into management server but it doesn't show up as online
Log shows something like this:
```
DEBG client/internal/login.go:93: connecting to the Management service https://api.openzro.io:443
DEBG client/internal/login.go:63: connected to the Management service https://api.openzro.io:443
DEBG client/internal/login.go:93: connecting to the Management service https://api.openzro.io:443
DEBG client/internal/login.go:63: connected to the Management service https://api.openzro.io:443
INFO client/internal/connect.go:119: starting openZro client version 0.28.6 on linux/amd64
DEBG client/internal/connect.go:180: connecting to the Management service api.openzro.io:443
DEBG client/internal/connect.go:188: connected to the Management service api.openzro.io:443
DEBG signal/client/grpc.go:81: connected to Signal Service: signal.openzro.io:443
...
ERROR util/grpc/dialer.go:38: Failed to dial: dial: dial tcp: lookup signal.openzro.io on 172.17.0.1:53: read udp 172.17.0.2:34638->172.17.0.1:53: i/o timeout
```
