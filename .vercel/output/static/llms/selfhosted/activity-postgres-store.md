...
Source: https://docs.netbird.io/selfhosted/activity-postgres-store
...
1. Backup your data store (`events.db` in `datadir` - default `/var/lib/openzro/`)
```bash
mkdir backup
...
```bash
docker compose cp backup/events.db management:/var/lib/openzro/events.db
```
