...
Source: https://docs.netbird.io/selfhosted/postgres-store
...
You can also pass the DSN as an `NB_` environment variable on the `openzro-server` container in your `docker-compose.yml`:
...
```bash
docker compose restart openzro-server
...
> **Note:** Switching between storage options requires migration steps to prevent data loss.
...
1. Backup your data store (`store.db` in `datadir` - default `/var/lib/openzro/`)
...
> **Note:** As of version 0.26.0, openZro used SQLite as its default database store. However, SQLite lacked support for cascading deletions,
...
For the combined setup:
```bash
docker compose cp backup/. openzro-server:/var/lib/openzro/
```

For the older multi-container setup:
```bash
docker compose cp backup/. management:/var/lib/openzro/
```
...
This is optional and should be used only if you want to rollback the data from Postgres to SQLite while running the same openZro version.
...
For the combined setup:
```bash
docker compose cp store.db openzro-server:/var/lib/openzro/store.db
```

For the older multi-container setup:
```bash
docker compose cp store.db management:/var/lib/openzro/store.db
```
