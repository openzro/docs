...
Source: https://docs.netbird.io/selfhosted/migration/enable-reverse-proxy

---

# Migration Guide: Enable Reverse Proxy Feature

This guide walks you through adding the openZro Reverse Proxy to an existing self-hosted deployment. By the end, you'll have a `openzro-proxy` container running alongside your existing services, ready to expose internal applications to the public internet.

> **Note:** **Who is this guide for?** This migration guide is for existing self-hosted users who:
- Already have a working openZro deployment (management server, dashboard, signal, relay)
...
The openZro proxy container manages its own TLS certificates (via Let's Encrypt or static files). This means the reverse proxy sitting in front of it **must not terminate TLS** - it needs to pass raw TLS connections through to the proxy container untouched.
...
- **`proxy` service (container: `openzro-proxy`)** - a new service in your Docker Compose stack that handles TLS termination, certificate provisioning, and traffic forwarding for reverse proxy services
...
- Your existing openZro services are unchanged
...
- **Latest openZro images** - pull the latest version to ensure the management server and CLI support the reverse proxy feature and token creation
- **A domain for the proxy** - e.g., `proxy.example.com` or `openzro.example.com` (it can be the same domain as the openZro server itself). Service subdomains will be created under this domain (e.g., `myapp.openzro.example.com`)
- **Wildcard DNS capability** - ability to create a `*.openzro.example.com` DNS record pointing to your server
- **Port 443 accessible** - the proxy needs this for ACME TLS-ALPN-01 challenges (certificate provisioning)

> **Note:** This guide covers both the **combined container** setup (`openzro/openzro-server`, the default for new deployments) and the **multi-container** setup (separate `management`, `signal`, and `relay` images). Where commands or configuration differ between the two setups, both variants are shown.
...
mkdir -p openzro-backup-$(date +%Y%m%d)
cd openzro-backup-$(date +%Y%m%d)
...
**Combined container** (`openzro/openzro-server`):

```bash
docker exec -it openzro-server /go/bin/openzro-server token create \
  --name "my-proxy" --config <openzro-data-dir>/config.yaml
```

**Multi-container** (separate `openzro/management` image):

```bash
docker exec -it openzro-management /go/bin/openzro-mgmt token create --name "my-proxy"
```
...
docker exec -it openzro-server /go/bin/openzro-server token list \
  --config <openzro-data-dir>/config.yaml

# List all tokens (multi-container)
docker exec -it openzro-management /go/bin/openzro-mgmt token list

# Revoke a token by ID (combined container)
docker exec -it openzro-server /go/bin/openzro-server token revoke <token-id> \
  --config <openzro-data-dir>/config.yaml

# Revoke a token by ID (multi-container)
docker exec -it openzro-management /go/bin/openzro-mgmt token revoke <token-id>
```
...
```yaml
proxy:
  image: openzro/reverse-proxy:latest
  container_name: openzro-proxy
  restart: unless-stopped
  networks: [openzro]
  depends_on:
    - openzro-server  # Use "management" for multi-container setup
  env_file:
    - ./proxy.env
  volumes:
    - openzro_proxy_certs:/certs
  labels:
    - traefik.enable=true
...
Also add the `openzro_proxy_certs` volume to your `volumes:` section:
...
NB_PROXY_MANAGEMENT_ADDRESS=http://openzro-server:80
...
> **Note:** The proxy connects to the management server directly over the Docker network rather than through Traefik; this avoids the need to route the `/management.ProxyService/` gRPC service through your reverse proxy. The `NB_PROXY_ALLOW_INSECURE=true` setting is safe here because the traffic never leaves the Docker network.
...
- Uses `priority=1` to avoid intercepting traffic meant for the main openZro HTTP routers on the same entrypoint
- Forwards traffic to the proxy container on port 8443

> **Note:** The `HostSNI(*)` rule acts as a catch-all for any domain not matched by the existing openZro HTTP routers. The `priority=1` ensures this TCP router only handles traffic that no other router claims. Any domain pointing to your server that isn't `openzro.example.com` will be forwarded to the proxy.
...
> **Note:** You only need port mappings for TCP and UDP mode services. HTTP and TLS mode services are routed through port 443 via Traefik and do not require additional port entries.

### Step 4: Set up DNS records

Create one DNS record pointing to the server running your openZro stack - one for the base proxy domain and one wildcard for service subdomains:

| Type | Name | Content |
|------|------|---------|
| `CNAME` | `*.openzro.example.com` | `openzro.example.com` |

The base domain record is required because a wildcard DNS record does not cover the bare domain itself. The wildcard record ensures that all service subdomains (e.g., `myapp.openzro.example.com`, `dashboard.openzro.example.com`) resolve to your server where Traefik forwards them to the proxy container.
...
[CrowdSec](https://www.crowdsec.net) is an open-source security engine that shares threat intelligence across its community network. When integrated with the openZro Proxy, it checks every incoming client IP against a local cache of known malicious addresses and blocks them before they reach your services. This step is entirely optional.

> **Note:** **Already have the proxy running?** If you completed steps 1-6 previously and are coming back to add CrowdSec, start here. The instructions below work whether you're setting up the proxy for the first time or adding CrowdSec to an existing proxy deployment.
...
```yaml
  crowdsec:
    ...
    container_name: openzro-crowdsec
    restart: unless-stopped
    networks: [openzro]
    environment:
...
```bash
docker compose exec crowdsec cscli bouncers add openzro-proxy -o raw
```
...
> **Note:** In **enforce** mode, if the bouncer has not completed its initial sync with the LAPI, all connections to that service will be denied. This is by design (fail-closed). If you want to avoid this during initial rollout, start with **observe** mode.
...
Replace `<your-management-domain>` with your openZro management server domain (the same domain used for the dashboard). Replace `<your-auth-client-id>` with the OAuth2 client ID from your identity provider (the same client ID used for the dashboard application).
...
> **Note:** Migrating from a standalone external IdP to the embedded IdP with your IdP as a connector requires user ID migration. See the [Migration Guide](/selfhosted/identity-providers#migration-guide-and-backwards-compatibility) or contact [support@openzro.io](mailto:support@openzro.io) for assistance.
...
> **Note:** If your proxy and management server run on **separate hosts** - which is the typical reason for routing through Traefik - you can skip this step. The proxy will resolve your management domain to the remote host's public IP via normal DNS.
...
services:
  ...
  proxy:
    ...
    extra_hosts:
      - "openzro.example.com:172.30.0.10"
```

Replace `openzro.example.com` with your actual management domain.
...
```bash
NB_PROXY_MANAGEMENT_ADDRESS=https://openzro.example.com:443
# Do NOT set NB_PROXY_ALLOW_INSECURE when connecting over TLS through Traefik
```

> **Note:** If you use the `extra_hosts` approach above, you **must** assign a static IP to Traefik. Without it, Docker may assign a different IP on container restart and the `extra_hosts` entry will silently point to the wrong address.

## For users not on Traefik

If your self-hosted deployment currently uses Nginx, Caddy, or another reverse proxy, you'll need to switch to Traefik before enabling the Reverse Proxy feature. See the [Traefik setup instructions](/selfhosted/external-reverse-proxy#traefik) for a step-by-step guide on configuring Traefik for your openZro deployment.

## Environment variable reference

| Variable | Required | Description | Default |
|----------|----------|-------------|---------|
| `NB_PROXY_TOKEN` | Yes | Access token generated via `openzro-server token create` (combined) or `openzro-mgmt token create` (multi-container). The proxy refuses to start without it. | - |
| `NB_PROXY_DOMAIN` | Yes | Base domain for this proxy instance (e.g., `proxy.example.com`or `openzro.example.com`). Determines the domain available for services. | - |
| `NB_PROXY_MANAGEMENT_ADDRESS` | No | URL of your openZro management server. The proxy connects via gRPC to register itself. | `https://api.openzro.io:443` |
...
4. Verify the TCP router has `priority=1` to prevent it from intercepting traffic meant for the main openZro HTTP routers
...
- [Self-Hosting Quickstart](/selfhosted/selfhosted-quickstart) - getting started with self-hosted openZro