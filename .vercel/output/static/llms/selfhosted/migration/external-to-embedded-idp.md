...
Source: https://docs.netbird.io/selfhosted/migration/external-to-embedded-idp

---

# Migration Guide: External IdP to Embedded IdP

This guide walks through migrating a self-hosted openZro deployment from an external identity provider to the embedded IdP introduced in v0.62.0.
...
> **Note:** This guide assumes you're using Docker and everything is run in the same bash session, if you're using a different setup, adjust the commands accordingly.

### Prerequisites

| Requirement | Details |
|-------------|---------|
| openZro version | v0.67.2 or later |
...
> **Note:** When to use --domain vs --dashboard-domain vs --api-domain:
...
- If you don't have a reverse proxy in front of dashboard and API, make sure you use the domain + port combination for each.
  For example, `--dashboard-domain demo.openzro.io` and `--api-domain demo.openzro.io:33073`
...
> **Note:** Before starting the migration, it's also a good idea to log out of the dashboard, as you might get a "stale" token from the old IdP which can cause 401 errors.
...
curl -L -o openzro-idp-migrate.tar.gz \
  https://github.com/openzro/openzro/releases/download/v0.67.2/openzro-idp-migrate_0.67.2_linux_amd64.tar.gz
tar xzf openzro-idp-migrate.tar.gz
chmod +x openzro-idp-migrate
```
...
```bash
go build -o openzro-idp-migrate ./tools/idp-migrate/
```
...
> **Note:** Using Zitadel as an example, the JSON should have the following values:
...
> **Note:** Do not skip this step. The migration modifies user IDs in the database. A manual backup is your only recovery path if something goes wrong.
...
```bash
./openzro-idp-migrate --domain mgmt.example.com --dry-run
```

> **Note:** If the env vars are not set, you can use the flags listed at the [Flags and Environment variables](#flags-and-environment-variables) section.
...
```bash
./openzro-idp-migrate --domain mgmt.example.com
```
...
> **Note:** Use an incognito/private browser window or clear cookies for your first login. Stale tokens from the old IdP will fail validation.
