...
Source: https://docs.netbird.io/selfhosted/migration/combined-container

---

# Migration Guide: Combined Container Setup

This guide walks you through migrating a pre-v0.65.0 openZro self-hosted deployment from the old 5-container architecture (dashboard, signal, relay, management, coturn) to the new 2-container combined setup (Traefik + `openzro-server`). The migration is handled by the `migrate.sh` script, which automates detection, backup, configuration generation, and (for Caddy-based setups) the full cutover.

> **Note:** **Who is this guide for?** This migration guide is for users who:
...
- Want to move to the combined `openzro-server` container for a simpler, more maintainable deployment

> **Note:** The migration script **exits with an error** if it detects an external identity provider (Auth0, Keycloak, Okta, Zitadel, Google Workspace, Microsoft Entra ID, etc.). If you are using an external IdP, first follow the [External IdP to Embedded IdP migration guide](/selfhosted/migration/external-to-embedded-idp) to switch to the embedded Dex IdP, then return here to complete the combined container migration.

## Overview of changes

### What's new

- **Combined `openzro-server` container** - management, signal, relay, and the embedded STUN server all run in a single container, replacing four separate services
...
- **Simplified Docker Compose** - the stack goes from 5+ services down to 2 (Traefik + `openzro-server`) or 3 (with dashboard)

### What's removed

- **Separate signal, relay, management, and coturn containers** - consolidated into `openzro-server`
...
- **Root or sudo access** to the machine running your openZro deployment
...
**Phase 0 - Detection:** The script locates your installation directory (checking `$PWD`, `/opt/openzro`), validates the old setup by looking for `management.json` and `docker-compose.yml`, and detects your reverse proxy type (embedded Caddy, Traefik, or external), IdP type, Docker volumes, domain, store engine (SQLite/PostgreSQL/MySQL), encryption key, and relay secret. If `config.yaml` already exists, the script exits - it assumes you have already migrated.
...
```bash
curl -fsSL https://github.com/openzro/openzro/raw/main/infrastructure_files/migrate.sh -o migrate.sh
chmod +x migrate.sh
```

### Step 2: Run the migration

If your openZro installation is in the current directory:
...
```bash
./migrate.sh --install-dir /opt/openzro
```
...
```bash
./migrate.sh --install-dir /opt/openzro --non-interactive
```
...
cd /opt/openzro  # or your install directory
docker compose ps

# Check logs
docker compose logs -f openzro-server
...
> **Note:** The gRPC routes require HTTP/2 cleartext (h2c) support in your reverse proxy. Ensure your proxy is configured to forward these paths using h2c, not standard HTTP/1.1.
...
| Flag | Description |
|---|---|
| `--install-dir DIR` | Path to the existing openZro installation directory. If not specified, the script checks `$PWD`, `/opt/openzro`, or prompts interactively. |
...
If the script cannot find `management.json` in any of the default locations (`$PWD`, `/opt/openzro`), use the `--install-dir` flag to specify the path explicitly:

```bash
./migrate.sh --install-dir /path/to/your/openzro
```
...
bash /opt/openzro/backup-YYYYMMDD-HHMMSS/rollback.sh
```
...
- [Enable Reverse Proxy](/selfhosted/migration/enable-reverse-proxy) - after migrating to the combined container setup with Traefik, follow this guide to add the openZro Reverse Proxy service
...
- [migrate.sh source](https://github.com/openzro/openzro/blob/main/infrastructure_files/migrate.sh) - the migration script on GitHub