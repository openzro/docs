...
Source: https://docs.netbird.io/selfhosted/migration/coturn-to-stun-migration

---

# Migration Guide: From Coturn to Embedded STUN Server

This guide helps you migrate your existing openZro self-hosted installation from using an external Coturn TURN server to the new embedded STUN server integrated into the relay service.

> **Note:** This guide applies only to the **older multi-container setup** with separate `management`, `signal`, `relay`, and `coturn` containers configured via `management.json`. If you deployed using the [`getting-started.sh`](/selfhosted/selfhosted-quickstart) script (combined setup with `config.yaml`), STUN is already embedded — no migration is needed. If you are still on the older setup, consider migrating to the [combined container](/selfhosted/migration/combined-container) instead, which handles this automatically.

> **Note:** **Who is this guide for?** This migration guide is intended for users who:
- Have an existing openZro self-hosted deployment with **Coturn configured for STUN** (NAT traversal)
- Already have the **openZro Relay service** running for relayed connections

If you were using Coturn purely for TURN relay functionality, note that openZro Relay has replaced TURN for relayed connections. This guide focuses on migrating the STUN functionality from Coturn to the embedded STUN server in the relay service.
...
- **Better Integration**: STUN server is tightly integrated with the openZro relay service
...
- openZro [relay image](https://hub.docker.com/r/openzro/relay) updated to the latest version
...
> **Note:** Keep your existing `NB_AUTH_SECRET` value from your current configuration. If using multiple STUN ports, ensure all ports are exposed in your `docker-compose.yml` and listed in `NB_STUN_PORTS`.
...
```json
{
    ...
    "Relay": {
        "Addresses": ["rels://your-relay-domain.com:443"], // openZro Relay domain and port
        "CredentialsTTL": "24h",
...
```json
{
    "Stuns": [
        {
            "Proto": "udp",
            "URI": "stun:your-relay-domain:3478" // openZro Relay domain and port
        }
    ],
    "Relay": {
        "Addresses": ["rels://your-relay-domain.com:443"], // openZro Relay domain and port
...
> **Note:** Remove the `TURNConfig` section entirely as TURN is replaced by the openZro relay service.
...
1. **Check STUN Server**:
   ...
   ```bash
   docker logs --tail 100 -f openzro-relay
   ```
...
```bash
curl -sSL https://github.com/openzro/openzro/raw/main/infrastructure_files/getting-started.sh | bash
```
...
- [openZro Self-Hosting Quickstart](/selfhosted/selfhosted-quickstart)
- [PR #5062: Embedded STUN Implementation](https://github.com/openzro/openzro/pull/5062)