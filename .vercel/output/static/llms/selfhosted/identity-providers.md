...
Source: https://docs.netbird.io/selfhosted/identity-providers
...
openZro's self-hosted implementation uses the OpenID Connect (OIDC) protocol for authentication, an industry-standard
...
> **Note:** While we maintain a list of 'supported' (tested) IdPs, **any OIDC provider should work with openZro's 'OIDC (Generic)' connector**.

## How Authentication Works in openZro

When a user attempts to access the openZro dashboard from a web browser or an internal resource from their device,
...
By default, openZro does not require an external identity provider. New installations include a built-in authentication
...
openZro supports connecting **multiple external identity providers** [alongside local user management](/selfhosted/identity-providers/local).
...
openZro supports any OIDC-compliant identity providers. Here are some popular providers and their setup instructions:
...
> **Note:** Any identity provider that supports OpenID Connect can be integrated with openZro. If your provider isn't listed above,
...
curl "https://openzro.example.com/api/identity-providers" \
  -H "Authorization: Bearer ${TOKEN}"

# Update an identity provider
curl -X PUT "https://openzro.example.com/api/identity-providers/{id}" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Updated Name"
...
curl -X DELETE "https://openzro.example.com/api/identity-providers/{id}" \
  -H "Authorization: Bearer ${TOKEN}"
```
...
3. After successful authentication, user is redirected back to openZro
4. openZro validates the token and creates/updates the user account
...
If you've connected an external IdP, openZro can optionally fetch a user's groups via JWT claim. These groups automatically obtain representations within openZro and will be applied to the corresponding openZro user. To enable JWT group sync, 
navigate to Settings > Groups and toggle 'Enable JWT group sync'. 

    

Specify the JWT claim to be used as the user's groups list (normally 'groups'). You can optionally specify a 'JWT allow groups' - this group will need to exist in your chosen claim for the user in order for that user to be granted access to openZro. 
Your IdP may require specific configuration in order to pass a groups claim to openZro. For detailed per-IdP implementation steps, see below. If your IdP isn't on the list, refer to the project's documentation. 
...
> **Note:** Groups with matching names in openZro and your IdP will **not** sync. To import a group from your IdP, first delete the existing group with that name in openZro.

#### SCIM 
openZro supports provisioning users and groups through SCIM. However, this functionality is not available in the open source Community Edition. It is offered only in the cloud-managed version of openZro or through a [Commercial License](https://openzro.io/pricing#on-prem) for enterprise self-hosted deployments.
...
- Copy the exact Redirect URL from openZro after creating the provider
...
If you have an existing openZro deployment using a standalone IdP (like Zitadel from the previous quickstart), you have several options:
...
> **Note:** We are working on a migration tool to automate the process of switching from standalone IdPs to external providers.
    Please contact us at [](mailto:support@openzro.io) if you are interested in this feature.