...
openZro is open-source and can be self-hosted on your servers.

It relies on components developed by openZro Authors [Management Service](https://github.com/openzro/openzro/tree/main/management), [Management UI Dashboard](https://github.com/openzro/dashboard), [Signal Service](https://github.com/openzro/openzro/tree/main/signal),
a 3rd party open-source STUN/TURN service [Coturn](https://github.com/coturn/coturn), and an identity provider (IDP).

If you would like to learn more about the architecture please refer to the [architecture section](/about-openzro/how-openzro-works).

> **Note:** This guide uses the **older multi-container architecture** with separate `management`, `signal`, `relay`, and `coturn` containers, configured via `management.json`. For new deployments, the [Quickstart guide](/selfhosted/selfhosted-quickstart) uses the **combined setup** with a single `openzro-server` container and `config.yaml`. If you are on the older setup, see the [migration guide](/selfhosted/migration/combined-container) to upgrade.

> **Note:** **New to self-hosting?** The [Quickstart guide](/selfhosted/selfhosted-quickstart) uses the built-in identity provider and is the fastest way to get started. This advanced guide is for users who need to integrate with an existing IdP or have specific enterprise requirements.

> **Note:** It might be a good idea to try openZro before self-hosting on your servers.
We run openZro in the cloud, and it will take a few clicks to get started with our managed version. [Check it out!](https://openzro.io/pricing)
...
Since version 0.29, openZro uses a consolidated port architecture where Management and Signal services share ports via HTTP/2 protocol negotiation. This significantly reduces the number of ports you need to open.
...
> **Note:** The default setup script configures a Caddy reverse proxy that consolidates all services behind ports 80 and 443. If you're running openZro behind your own reverse proxy, see the [advanced configuration](#advanced-running-net-bird-behind-an-existing-reverse-proxy) section.

> **Note:** **Legacy port requirements (pre-v0.29):** TCP `80, 443, 33073, 10000, 33080` plus UDP `3478` and UDP `49152-65535` (Coturn TURN relay port range). If you have any clients running versions below v0.29, you must keep these legacy ports open even if your server is upgraded - older clients cannot use the consolidated ports.

For this tutorial we will be using domain ```demo.openzro.io``` which points to our Ubuntu 22.04 machine hosted at Hetzner.

### Step 1: Get the latest stable openZro code

```bash
#!/bin/bash
REPO="https://github.com/openzro/openzro/"
...
To simplify the setup we have prepared a script to substitute required properties in the [docker-compose.yml.tmpl](https://github.com/openzro/openzro/tree/main/infrastructure_files/docker-compose.yml.tmpl) and [management.json.tmpl](https://github.com/openzro/openzro/tree/main/infrastructure_files/management.json.tmpl) files.

The [setup.env.example](https://github.com/openzro/openzro/tree/main/infrastructure_files/setup.env.example) file contains multiple properties that have to be filled. You need to copy the example file to `setup.env` before updating it.
...
> **Note:** The changes made to `setup.env` will not take any effect until you run `configure.sh` (mentioned later on).

- Set ```NETBIRD_DOMAIN``` to your domain, e.g.  `demo.openzro.io`
- Configure ```NETBIRD_LETSENCRYPT_EMAIL``` property.
This can be any email address. [Let's Encrypt](https://letsencrypt.org/) will create an account while generating a new certificate.

> **Note:** openZro supports automatic renewal by default when using Let's Encrypt.

> **Note:** If you want to setup openZro with your own reverse-Proxy and without using the integrated letsencrypt, follow [this step here instead](#advanced-running-net-bird-behind-an-existing-reverse-proxy).

### Step 3: Configure Identity Provider (IDP)

openZro supports generic OpenID (OIDC) protocol allowing integration with any IDP following the specification.

openZro's management service integrates with some of the most popular IDP APIs, allowing the service to cache and display user names and email addresses without storing sensitive data.
...
> **Note:** Each provider page includes both "Management Setup (Recommended)" (for use with the embedded IdP) and "Standalone Setup (Advanced)" sections. For this guide, follow the **Standalone Setup (Advanced)** section.

### Step 4: Disable single account mode (optional)

openZro Management service runs in a single account mode by default since version v0.10.1.
...
[docker-compose.yml.tmpl](https://github.com/openzro/openzro/tree/main/infrastructure_files/docker-compose.yml.tmpl)
...
> **Note:** The changes made to `setup.env` will not take any effect until you run `configure.sh` again

> **Note:** For detailed documentation of all configuration file options, see the [Configuration Files Reference](/selfhosted/configuration-files).
...
## Advanced: Running openZro behind an existing reverse-proxy

If you already have a reverse proxy (Nginx, Caddy, etc.), you can configure openZro to work with it instead of using the built-in Traefik.

> **Note:** Not all reverse proxies are supported as openZro uses *gRPC* for various components. Your reverse proxy must support HTTP/2 and gRPC proxying.

**New deployments:** The `getting-started.sh` script can simplify the integration of openZro into your existing reverse proxy setup by generating a drop-in config or bespoke instructions.
...
```
More info can be found at this GitHub [issue](https://github.com/openzro/openzro/issues/390#issuecomment-1185298689).
...
```
> **Note:** Oracle Cloud discourages use of UFW. You will have to use IPTABLES.

## Backup
To backup your openZro installation, you need to copy the configuration files, and the Management service databases.
...
```bash
cd openzro/infrastructure_files/artifacts/
mkdir backup
...
## Upgrade
> **Note:** The users with management on version

To upgrade openZro to the latest version, you need to review the [release notes](https://github.com/openzro/openzro/releases) for any breaking changes and follow the upgrade steps below:
1. Run the backup steps described in the [backup](#backup-net-bird) section.
2. Pull the latest openZro docker images:
   ```bash
   cd openzro/infrastructure_files/artifacts/
   docker compose pull
    ```
3. Restart the openZro containers with the new images:
   ```bash
...
If you've deployed openZro using this advanced guide and want to simplify your setup by migrating to the embedded IdP:

1. Your existing IdP can be added as an embedded IdP directly in the openZro Management Dashboard
