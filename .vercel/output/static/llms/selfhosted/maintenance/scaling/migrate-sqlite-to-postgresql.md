...
Source: https://docs.netbird.io/selfhosted/maintenance/scaling/migrate-sqlite-to-postgresql
...
The default openZro deployment uses SQLite, which stores all data in a single file. This works well for smaller setups, but you may want to migrate to PostgreSQL if:
...
> **Note:** Replace the default `password` with a strong, unique password for production deployments.
...
> **Note:** Update the SQLite path and PostgreSQL connection string to match your environment.
...
```bash
docker compose logs openzro-server | grep store
```
...
```bash
docker exec postgres-server psql -U postgres -c "CREATE DATABASE openzro_auth;"
```
...
LOAD DATABASE
     ...
     INTO postgresql://postgres:password@localhost:5432/openzro_auth
...
server:
  ...
  authStore:
    engine: "postgres"
    dsn: "host=postgres-server port=5432 user=postgres password=password dbname=openzro_auth sslmode=disable"
```
...
```bash
docker exec postgres-server psql -U postgres -c "CREATE DATABASE openzro_events;"
```
...
LOAD DATABASE
     ...
     INTO postgresql://postgres:password@localhost:5432/openzro_events

WITH include drop, create tables, create indexes, reset sequences
;
```

> **Note:** No boolean CAST rules are needed for the activity store — all columns use standard integer and text types.
...
server:
  ...
  activityStore:
    engine: "postgres"
    dsn: "host=postgres-server port=5432 user=postgres password=password dbname=openzro_events sslmode=disable"
```
