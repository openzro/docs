...
Source: https://docs.netbird.io/selfhosted/maintenance/scaling/multiple-proxy-instances

---

# Running Multiple Proxy Instances

Running a single openZro proxy instance works well for many deployments, but production environments often require higher availability. By deploying multiple proxy instances across separate servers, you get automatic failover - if one instance goes down, the remaining instances continue serving traffic without interruption.
...
```
  ...
  │                  │  Proxy X (TLS)             │ ◄── WireGuard ──┤► openZro Peer
...
docker exec -it openzro-server /go/bin/openzro-server token create \
  --name "proxy-server-1" --config <openzro-data-dir>/config.yaml

docker exec -it openzro-server /go/bin/openzro-server token create \
  --name "proxy-server-2" --config <openzro-data-dir>/config.yaml
```

**Multi-container** (separate `openzro/management` image):

```bash
docker exec -it openzro-management /go/bin/openzro-mgmt token create --name "proxy-server-1"
docker exec -it openzro-management /go/bin/openzro-mgmt token create --name "proxy-server-2"
```
...
> **Note:** Tokens are displayed only once when created. Save each token immediately - the management server stores only a SHA-256 hash and cannot retrieve the original value.
...
> **Note:** On additional servers that only run the proxy (not the full openZro stack), the Traefik configuration is simpler because there are no competing HTTP routers. All traffic on port 443 is passed through to the proxy container.
...
> **Note:** The proxy watches the certificate files for changes and reloads them without a restart. This works seamlessly with shared storage - renew the certificate once on the NFS share, and all instances begin using the new certificate within seconds.
...
1. Open the openZro dashboard and navigate to **Reverse Proxy** > **Services**
...
- [Scaling Your Deployment](/selfhosted/maintenance/scaling/scaling-your-self-hosted-deployment) - overview of scaling options for self-hosted openZro
