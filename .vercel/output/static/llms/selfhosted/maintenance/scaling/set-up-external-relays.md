...
Source: https://docs.netbird.io/selfhosted/maintenance/scaling/set-up-external-relays
...
> **Note:** Replace `relay-us.example.com` with your relay server's domain and `your-shared-secret-here` with the secret you generated.
...
Now update your main openZro server to use the external relays instead of the embedded one.
...
```bash
cd ~/openzro  # or wherever your deployment is
nano config.yaml
```
...
server:
  listenAddress: ":80"
  exposedAddress: "https://openzro.example.com:443"
...
> **Note:** The `secret` under `relays` and the `NB_AUTH_SECRET` on all relay servers **must be identical**. Mismatched secrets will cause relay connections to fail silently.
...
Connect a openZro client and verify that both STUN and relay services are available:
...
```bash
sudo openzro service reconfigure --service-env NB_FORCE_RELAY=true
```
...
```bash
sudo openzro service reconfigure --service-env NB_FORCE_RELAY=false
```