...
Source: https://docs.netbird.io/selfhosted/identity-providers/local

---

# Local User Management

openZro's Management service includes built-in user management, allowing you to create and manage local users directly without requiring an external identity provider. This functionality is powered by an embedded [Dex](https://dexidp.io/) server.

Starting with version 0.62, openZro **no longer requires an external identity provider**. The Management service now supports creating and managing local users directly, so you can get started without setting up Zitadel, Keycloak, or any other IdP.

With local user management, you can:

- **Create local users** directly from the openZro Dashboard
...
> **Note:** Local user management is powered by an embedded [Dex](https://dexidp.io/) server running within the openZro Management service, requiring no additional containers or databases.
...
> **Note:** This section applies to deployments using the older multi-container architecture. If you deployed using `getting-started.sh`, you are on the combined setup and should use the `config.yaml` instructions above. See the [migration guide](/selfhosted/migration/combined-container) to upgrade.
...
environment:
  ...
  - NETBIRD_EMBEDDED_IDP_DATA_DIR=/var/lib/openzro/idp
...
> **Note:** Store your encryption key securely. If lost, encrypted user data (emails, names) cannot be recovered. Include it in your backup procedures.
...
3. Log in to openZro with their new credentials
...
> **Note:** When an invite link is regenerated, the previous link becomes invalid. Only the new link can be used to complete registration.
...
> **Note:** The generated password is only shown once at creation time. It cannot be retrieved later. Make sure to copy it and share it securely with the user.

### Inviting Users via API

```bash
curl -X POST "https://openzro.example.com/api/users/invites" \
...
```
https://openzro.example.com/invite?token=abc123-token-xyz
```

### Changing User Passwords

The Change Password feature allows local users on self-hosted openZro deployments to update their account password directly from the dashboard.

    

This feature is available when:
  - The deployment is self-hosted (not openZro's hosted platform)
...
```bash
curl -X POST "https://openzro.example.com/api/users" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
...
> **Note:** The `password` field is only included when creating users with embedded IdP. Store it immediately—it won't be returned in subsequent API calls.
...
When openZro starts with the embedded IdP and no existing accounts, the Dashboard redirects to the `/setup` route and displays the **Instance Setup Wizard**:
...
> **Note:** The `/setup` route is unauthenticated and only accessible when `setup_required` is true. Once setup is complete, accessing `/setup` returns a 412 error and redirects to login.
...
curl "https://openzro.example.com/api/instance"
...
# Complete setup
curl -X POST "https://openzro.example.com/api/setup" \
  -H "Content-Type: application/json" \
  -d '{
...
1. Check server logs: `docker compose logs openzro-server` (or `docker compose logs management` for older multi-container setups)
2. Verify the encryption key hasn't changed
3. Confirm the user exists: Check **Team** → **Users** in Dashboard

## Disabling Embedded IdP

> **Note:** The embedded IdP cannot be disabled in the combined setup. If you want to use only external identity providers, you can [disable local authentication](/selfhosted/identity-providers/disable-local-authentication) instead and add your external IdP through the Dashboard UI.
...
> **Note:** Disabling the embedded IdP will invalidate all local user accounts. Ensure users have accounts in the external IdP before switching.