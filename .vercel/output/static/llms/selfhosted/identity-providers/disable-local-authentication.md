...
Source: https://docs.netbird.io/selfhosted/identity-providers/disable-local-authentication

---

# Disable Local Authentication

If you prefer to delegate all credential storage and authentication to your IdP while still utilizing openZro's new,
...
- Simplify end user login by directing users to your external IdP when logging in to openZro
- Prevent local password-based logins while still using openZro's simplified IdP connection flow
- Maintain a single source of truth for user credentials in your external IdP

> **Note:** Disabling local authentication preserves existing local user accounts in the database.
...
2. Log out and log in with your new admin account via the external IdP. openZro will notify you that the user requires approval.
3. Log back in as your original openZro-local admin and navigate to **Team > Users**. You should see the new IdP user pending approval:
...
> **Note:** Complete all prerequisite steps before disabling local authentication. If you disable local login without first
promoting an external IdP user to Owner, you will lose administrative access to your instance.

> **Note:** openZro will refuse to disable local authentication if no external identity provider connectors are configured.
...
> **Note:** If you deployed using the [`getting-started.sh`](/selfhosted/selfhosted-quickstart) quickstart script (post v0.65.0) or have
...
server:
  auth:
    issuer: "https://openzro.example.com/oauth2"
    localAuthDisabled: true
    signKeyRefreshEnabled: true
    dashboardRedirectURIs:
      - "https://openzro.example.com/nb-auth"
      - "https://openzro.example.com/nb-silent-auth"
    cliRedirectURIs:
...
> **Note:** This section applies to deployments using the older multi-container architecture with separate dashboard, management,
...
- **Combined setup:** set `localAuthDisabled` back to `false` in `config.yaml` and run `docker compose restart openzro-server`.
