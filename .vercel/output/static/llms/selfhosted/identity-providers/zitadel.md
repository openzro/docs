# Zitadel with openZro Self-Hosted

Source: https://docs.netbird.io/selfhosted/identity-providers/zitadel
...
Add Zitadel as an external IdP directly in the openZro Management Dashboard. This is the simplest approach for new deployments or when migrating from the previous quickstart.
...
3. Fill in the project details:
   - **Name**: `openZro` (or your preferred name)
...
5. openZro will display a **Redirect URL** — **copy this URL** (but don't click **Save** yet)
...
2. In the redirect URIs field, paste the redirect URL you copied from openZro
...
1. Return to the Zitadel Console tab
2. Navigate to your openZro application → **Token Settings**
3. Enable:
...
Zitadel uses **roles** instead of groups. By default, Zitadel's role claims use a nested JSON object format, but openZro expects a flat array of strings. You'll need to create a Zitadel Action to transform roles into a `groups` claim.
...
#### Step 4: Enable JWT Group Sync in openZro
...
> **Note:** The action transforms Zitadel's role structure into a flat array. If you need to include project context, modify the script to use `groups.push(role + ':' + claim.projectId)`.
...
For detailed instructions on the standalone setup, see the [Zitadel SSO with openZro Self-Hosted (Advanced)](/selfhosted/identity-providers/advanced/zitadel) documentation.
