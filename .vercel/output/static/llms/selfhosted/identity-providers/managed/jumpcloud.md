# JumpCloud SSO with openZro Self-Hosted

Source: https://docs.netbird.io/selfhosted/identity-providers/managed/jumpcloud
...
Add JumpCloud as an external IdP directly in the openZro Management Dashboard. This is the simplest approach and recommended for most deployments.
...
10. On the **SSO** tab, configure:
    - **Redirect URIs**: `https://<your-openzro-domain>/oauth2/callback` (you'll verify this matches exactly in Step 3)
    - **Client Authentication Type**: `Client Secret POST`
    - **Login URL**: `https://<your-openzro-domain>/` 
11. Under **Attribute Mapping**, enable:
    - **Email** scope
    - **Profile** scope
12. Click **Save** to save the SSO configuration
13. Click the **User Groups** tab and select at least one user group that can access openZro
14. **Don't click Activate yet** — keep this tab open and proceed to Step 2

> **Note:** Sometimes, the JumpCloud application configuration will add duplicate attributes, like `email` and `email_verified`. Remove any duplicates if they appear.
...
> **Note:** **Important:** The Issuer must be exactly `https://oauth.id.jumpcloud.com/` (with trailing slash) to match what JumpCloud returns. If you enter it without the trailing slash, the connector will fail to initialize.
...
2. Click the **SSO** tab
3. Under **Redirect URIs**, verify the redirect URL matches the exact URL you copied from openZro (e.g., `https://openzro.hopkins.sh/oauth2/callback`). If it doesn't match exactly, update it to match.
...
To sync JumpCloud groups with openZro, you need to enable the group attribute in your JumpCloud OIDC application.
...
2. Select your openZro application
3. Go to the **SSO** tab
...
1. In your openZro application, go to the **User Groups** tab
2. Select the groups whose members should have access to openZro
3. Click **Save**
...
> **Note:** **Known issue:** If a user belongs to only one group, JumpCloud may return it as a string instead of an array, which can cause issues. Ensure users are members of at least two groups for consistent behavior, or test with your specific setup.
...
For detailed instructions on the standalone setup, see the [JumpCloud SSO with openZro Self-Hosted (Legacy)](/selfhosted/identity-providers/managed/advanced/jumpcloud) documentation.
