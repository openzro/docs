# Microsoft and Entra ID SSO with openZro Self-Hosted

Source: https://docs.netbird.io/selfhosted/identity-providers/managed/microsoft-entra-id

---

# Microsoft and Entra ID SSO with openZro Self-Hosted

Use Microsoft accounts for authentication with openZro. This supports both personal Microsoft accounts and Microsoft Entra ID (formerly Azure AD) for work and school accounts.

## Management Setup (Recommended)

Add Microsoft as an external IdP directly in the openZro Management Dashboard. Choose the appropriate identity provider type based on your needs:
...
> **Note:** **Issuer URL format:** `https://login.microsoftonline.com/{tenant-id}/v2.0` where `{tenant-id}` is your Directory (tenant) ID from Azure.
...
4. Paste the redirect URL you copied from openZro in the **Redirect URI** field
...
To sync Entra ID groups with openZro, you need to configure your app registration to include group claims in the ID token.
...
> **Note:** **Token size limit:** Entra ID limits groups to 200 in JWT tokens. If a user belongs to more groups, the claim is omitted entirely. Use **Groups assigned to the application** to avoid this by only including relevant groups.
...
2. Find and select your openZro application
3. Go to **Users and groups**
4. Click **Add user/group**
5. Select the groups you want to sync with openZro
6. Click **Assign**
...
> **Note:** Entra ID returns group **object IDs** (GUIDs) by default, not group names. Your openZro groups will appear as IDs like `a1b2c3d4-5678-90ab-cdef-1234567890ab`. To use display names instead, you need Azure AD Premium and cloud-only groups. [openZro Cloud](https://openzro.io/) does not have this limitation—it syncs group names directly via the Microsoft integration.

---

## Standalone Setup (Advanced)

Use Microsoft Entra ID as your primary identity provider instead of openZro's embedded IdP. This option gives you full control over authentication and user management, is recommended for experienced Microsoft Entra ID administrators as it also requires additional setup and ongoing maintenance.
...
For detailed instructions on the standalone setup, see the [Microsoft and Entra ID SSO with openZro Self-Hosted (Legacy)](/selfhosted/identity-providers/managed/advanced/microsoft-entra-id) documentation.
...
- Ensure the redirect URI in Azure exactly matches what openZro provides
