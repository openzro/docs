...
Welcome to openZro! This guide will walk you through our new onboarding process to create your account, connect your first devices,
...
First, let's create your openZro account.

![openZro IdP SSO and MFA](/docs-static/img/get-started/onboarding/01_openzro-sso.jpeg)

1. Navigate to [openzro.io](https://openzro.io/) and click Get Started in the top-right corner. Or simply click [here](https://your-management.example.com/).
2. You’ll be redirected to the sign-in page, where openZro uses your identity provider (IdP) for secure authentication.
...
One way of using openZro is to create a peer-to-peer network, where you run the openZro client on your devices to connect them directly.
...
![Download openZro](/docs-static/img/get-started/onboarding/03_download-openzro.jpeg)

1. On the "Let's get your first device online" screen, click the Install openZro button.
2. An [installation modal](https://your-management.example.com/) will appear. Select your operating system (e.g., macOS, Windows, Linux). For this example, we're installing it on a macOS machine.
...
![Connect openZro Client](/docs-static/img/get-started/onboarding/04_connect-client.jpeg)

1. After installation, find the openZro icon in your system tray or menu bar.
...
4. Once authorized, you will see a "Login successful" message. The onboarding UI will update to show that your first peer is connected, displaying its name and assigned openZro IP address.

### Add a Second Peer (Headless Linux Server)
Next, let's add a second, headless peer, like a Linux server or a Raspberry Pi. For devices without a graphical interface, we use a [Setup Key](https://docs.openzro.io/manage/peers/register-machines-using-setup-keys).

![Install openZro Headless](/docs-static/img/get-started/onboarding/05_headless-installed.jpeg)
...
* **Install openZro**: A curl command to download and run the installation script.
* **Run openZro**: A [openzro up command](https://docs.openzro.io/get-started/cli) that includes your unique setup key.
...
Next, copy the openzro up --setup-key ... command and paste it into the terminal.

```bash
openzro up --setup-key <YOUR_SETUP_KEY>
```

After running the second command, the terminal will confirm Connected. Your headless device is now part of your openZro network.
...
![Testing ping on openZro](/docs-static/img/get-started/onboarding/07_ping-test.jpeg)

1. Copy the provided ping command from the onboarding UI. This command uses the openZro IP address of your second peer (the Ubuntu server).
2. Open a terminal on your first peer and paste the command. Replace the example below with the openZro IP for your machine.

```bash
ping 100.74.76.17
```

3. You should see successful ping replies, confirming that the two devices are connected over the openZro network. Click It works! - Continue in the onboarding UI.

### Understanding Access Control
The final onboarding step introduces openZro's powerful Access Control policies.

![openZro policy disabled](/docs-static/img/get-started/onboarding/08_policy-disabled-example.jpeg)
...
In the policy example above, we allowed _IT Admins_ port specific access to peers under the _AWS Servers_ group. Policies are a key building block to access in openZro. You can learn more about the power of policies [here](https://docs.openzro.io/manage/access-control/manage-network-access).

> **Note:** If you manage users and groups with your identity provider, you can provision and sync them with openZro. Learn more [here](https://docs.openzro.io/manage/team/idp-sync) including the supported platforms.

## Remote Network Access
The second way to use openZro is for remote network access by running openZro on a single machine within your private network.
This machine acts as a routing peer, routing traffic to internal resources that don't have the openZro client installed.
...
![openZro Onboarding](/docs-static/img/get-started/onboarding/10_remote-access-onboarding.jpeg)
...
![openZro Subnet Setup](/docs-static/img/get-started/onboarding/11_entire-subnet.jpeg)

### Add and Configure a Routing Peer
A [routing peer](https://docs.openzro.io/manage/network-routes) is a openZro peer that lives inside your private network and acts as a gateway, forwarding traffic between your remote users and the internal resources.

![Adding a routing peer](/docs-static/img/get-started/onboarding/12_add-routing-peer.jpeg)

1. The dashboard will now prompt you to "Add a routing peer." First, click Generate Setup Key. This creates a one-time key used to enroll the gateway machine into your openZro account.
2. Next, click Install Routing Peer. Select the operating system of your gateway machine (the video uses Linux).
3. The installation modal will provide two commands: a curl script to install the openZro agent and a openzro up command that includes your setup key.
...
1. Back in the web UI, the wizard will prompt you to "Time to add your client device." Click Install openZro.
2. Download and run the installer for your client machine's OS (e.g., macOS).
3. Once installed, find the openZro icon in your system tray or menu bar, click it, and select Connect.
...
Click Go to Dashboard to access the main openZro admin panel. From here, you can:

* [Control Center](https://docs.openzro.io/manage/control-center): Visualize your network topology and access relationships with an interactive graph.
* [Peers](https://docs.openzro.io/manage/peers/add-machines-to-your-network): View and manage all connected devices and their properties.
* [Setup Keys](https://docs.openzro.io/manage/peers/register-machines-using-setup-keys): Create and manage keys for adding new headless or ephemeral devices.
* [Access Control](https://docs.openzro.io/manage/access-control/manage-network-access): Define granular firewall rules to control which peers can access what.
* [Team](https://docs.openzro.io/manage/team/add-users-to-your-network): Manage users and create groups for easier policy management.

You are now ready to explore the full capabilities of openZro.
...
- openZro release page on GitHub: [releases](https://github.com/openzro/openzro/releases/latest)