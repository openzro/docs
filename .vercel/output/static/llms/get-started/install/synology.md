...
Source: https://docs.netbird.io/get-started/install/synology
...
Installing a openZro Peer on Synology will require a few additional steps compared to a typical Linux install despite using the same single line command to get everything going.
...
```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sudo sh
```

5. Add your Synology NAS as a Peer using the steps from [Add peers to your openZro network](https://docs.openzro.io/manage/peers/add-machines-to-your-network) in the documentation.

## Reboot Script

In some cases on Synology, the openZro service will not have the correct modules loaded on a reboot. In this case you’ll need to run a script every time you reboot your NAS. To automate this process, you can create a scheduled task in the Synology DSM.

1. _Control Panel > Task Scheduler > Create > Triggered Task > User defined script_.
2. Now you’ll have a _Create Task_ dialog box. Give a task name such as “openZro Reboot”. Set the _User > Root_ and the _Event > Boot-up_. Make sure the Enable Box is checked.
...
## Uninstallation
The most straightforward method is to use openZro's built-in uninstall command. You'll need to connect to your Synology NAS via SSH to execute these commands.
...
```
3. Remove openZro binary and configuration files.

/usr/local/bin/openzro: The installation script placed the openZro binary here.
/var/lib/openzro: This directory contains the openZro configuration files.

You can use the rm command to delete these files and directories:
```bash
rm /usr/local/bin/openzro
rm -rf /var/lib/openzro
```
...
4. Remove the Peer from the openZro UI.

For a complete cleanup, you should also remove the Synology NAS as a peer from your openZro account. Log in to the openZro web UI. Navigate to the Peers section. Find the peer corresponding to your Synology NAS and delete it.
