# Install openZro on Proxmox VE

Source: https://docs.netbird.io/get-started/install/proxmox-ve

---

# Install openZro on Proxmox VE

Proxmox VE (Virtual Environment) is an open-source server virtualization platform that combines KVM-based virtualization and LXC containerization. It provides a web-based management interface for deploying and managing virtual machines and containers, making it ideal for running the openZro agent in a containerized environment.

## Installing in an LXC

### Downloading a Container Template

Before creating an LXC container, you'll need to download a container template. openZro runs on a majority of Linux distributions, so you can choose any Linux template you're comfortable with. Popular choices include Ubuntu, Debian, CentOS, Alpine, and others.
...
Once we have a template ready, we can click the "Create CT" button on the top right. Then input all of your settings. If you're only using this container for openZro, the required specs will be relatively low and you can always change things in the future.
...
TUN provides an interface for user applications, such as openZro, to deal with the raw network traffic. By default, since we set this up as an unprivileged container, there is no access to this interface. To fix this, we'll need to bind the host's TUN device into the container and grant the container permission to use it. The lines below mount /dev/net and /dev/net/tun from the Proxmox host into the LXC and allow the TUN character device (10:200) so openZro can create virtual network interfaces inside the container.
...
Before we install openZro, we are going to want to generate a one-off setup key to use with our LXC installation. While [openZro's documentation offers comprehensive guidance on this process](https://docs.openzro.io/manage/peers/register-machines-using-setup-keys), let's quickly review the essential steps:

- Access your [openZro dashboard](https://your-management.example.com/)
...
Now that our system is updated, we can install openZro with the following command. 

```bash
curl -fsSL https://github.com/openzro/openzro/releases/latest | sh
```

Now that openZro is installed, we can connect our peer to our account using the setup key we generated earlier. 

```bash
  openzro up --setup-key <SETUP KEY>
```

Now you should see a successful connection message and you're good to go! Now if you want to access your entire network using this LXC with openZro, check out the docs to learn more about openZro Networks!
...
If you want to set up your own Proxmox machine, check out our [Knowledge Hub post on Proxmox](https://openzro.io/knowledge-hub/proxmox-getting-started-guide). This comprehensive guide covers the basics of getting Proxmox up and running, setting up a LXC container with openZro for remote connectivity, and diving into some more advanced tips and tricks such as setting up a ZFS pool and enabling IOMMU for GPU passthrough.