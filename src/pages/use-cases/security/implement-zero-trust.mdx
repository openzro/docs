...
### 0.1 Required features

openZro is open-source and self-host only — there are no licensing
tiers gating individual features. Every capability used in this
guide ships in the same management binary; you just need a
deployment running a recent enough version to pick them up.

This guide uses:

- **[User and group provisioning from your IdP](/manage/team/idp-sync)**
  for automatic group sync (Okta, Microsoft Entra, Google Workspace,
  JumpCloud, Keycloak, Auth0, Authentik, Zitadel, or generic SCIM).
- **[Audit event logging](/manage/activity)** for policy and
  configuration change history.
- **Device posture checks** to gate access based on OS version,
  client version, geolocation, CIDR ranges, running processes, and
  MDM/EDR compliance — see [Section 4.2](#42-posture-checks-and-their-limits)
  and the [Posture Checks guide](/manage/access-control/posture-checks).
- **Connection traffic event logging** for flow visibility — see
  [Section 7](#7-visibility-traffic-events-siem-and-control-center)
  and [Traffic Events](/manage/activity/traffic-events-logging).
  Requires the openZro client at v0.39 or newer.
- **Audit and traffic event streaming** to Datadog, Elastic, or any
  generic HTTP endpoint for SIEM integration — see
  [Section 7](#7-visibility-traffic-events-siem-and-control-center)
  and [Event Streaming](/manage/activity/event-streaming).
- [Access Control policies and groups](/manage/access-control),
  [Networks](/manage/networks), [Private DNS](/manage/dns), and the
  [Control Center](/manage/control-center) topology view.

Make sure your deployment's Management (the central coordination and
authentication service), Signal (the signaling service for peer
connection negotiation), and Relay (the traffic relay used when
direct connections fail) are all running supported versions
(preferably the [latest release](https://github.com/openzro/openzro/releases))
so these features all line up.
...
<Note>
For a complete list of openZro endpoints, IP addresses, and detailed firewall configuration examples, see the [FAQ: What firewall ports should I open to use openZro?](/about-openzro/faq#what-firewall-ports-should-i-open-to-use-openzro-network-firewalls)
</Note>
...
<p>
    <img src="/docs-static/img/about-openzro/high-level-dia.png" alt="openZro architecture showing Management, Signal, and Relay services" className="imagewrapper-big"/>
</p>

<Note>
- For a list of outbound ports required for openZro Cloud, see the [FAQ: What firewall ports should I open to use openZro?](/about-openzro/faq#what-firewall-ports-should-i-open-to-use-openzro-network-firewalls)
...
<p>
    <img src="/docs-static/img/manage/access-control/all-all-policy.png" alt="Default all-to-all policy in openZro UI" className="imagewrapper-big"/>
</p>
...
<p>
    <img src="/docs-static/img/manage/networks/openzro-network-routes.png" alt="high-level-dia" className="imagewrapper-big"/>
</p>
...
<p>
    <img src="/docs-static/img/manage/dns/openzro-dns-diagram.png" alt="openZro DNS resolution through routing peers" className="imagewrapper-big"/>
</p>
...
<Note>
Starting with openZro v0.59.0, the local DNS forwarder port changed from 5353 to 22054. The Management Service only applies the new port when all peers in the account run v0.59.0 or newer. If you have mixed client versions, DNS forwarding may behave differently across peers.
</Note>
