...
# Running openZro on serverless environments (FaaS)
...
Since [v0.25.3](https://github.com/openzro/openzro/releases), openZro enables secure connectivity and access from serverless functions like AWS lambda and Azure Functions to cloud or on-premises servers,
containers, databases, and other internal resources. openZro has adapted to the constraints of FaaS environments by leveraging netstack from
the [gVisor](https://github.com/google/gvisor) Go package, which is part of [Wireguard-go](https://github.com/openzro/wireguard-go),
...
You can enable the netstack mode for the openZro client using environment variables:
...
With these variables, openZro will launch a Socks5 proxy that you can use to connect to your internal resources.
...
### Docker
Some container environments can be restricted as well. For example, Docker containers are not allowed to create new VPN interfaces by default. For that reason, you can run a openZro agent in a standard mode to enable the netstack mode:
```bash
docker run --rm --name PEER_NAME --hostname PEER_NAME -d \
-e NB_SETUP_KEY=<SETUP KEY> -e NB_USE_NETSTACK_MODE=true -e  NB_SOCKS5_LISTENER_PORT=1080 -v openzro-client:/var/lib/openzro openzro/openzro:latest
```
...
def Example():
    ...
    # rest of the code...
```
## How to use openZro in FaaS environments?
...
There are many ways that you can configure these environments with openZro's client binary. We have created a simple example using containerized Azure Functions,
which you can find [Azure functions python db access example
](https://github.com/openzro/azure-functions-python-db-access).
