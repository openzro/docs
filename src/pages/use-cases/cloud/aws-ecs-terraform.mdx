...
## openZro Client on AWS ECS (Terraform)
...
**In this example, we will run openZro client configured as a daemon set in ECS deployed with Terraform.**

This allows you to:

* Run openZro as an ECS native service, you can manage and maintain it the same way you do with your other services.
* Connect to EC2 running on private subnets without the need to open firewall rules or configure bastion servers.
* Access other services connected to your openZro network and running anywhere.

### Requirements
* Terraform > 1.0.
* A openZro account with a Setup Key.
* Another openZro client in your network to validate the connection (possibly your laptop or a machine you are running this example on).
...
Login to https://app.openzro.io and [add your machine as a peer](https://app.openzro.io/add-peer), once you are done with the steps described there, copy your [Setup key](https://app.openzro.io/setup-keys).
...
After a few minutes, the autoscaling group will launch an EC2 instance and there you will find the openZro's ECS Daemon service running. With that, we can go to our [openZro dashboard](https://app.openzro.io) and pick the IP of the node that is running openZro, then we can connect to the node via ssh. For Unix(s) systems:
```shell
...
## openZro Client in Docker

One of the simplest ways of running openZro client application is to use a pre-built [Docker image](https://hub.docker.com/r/openzro/openzro).
...
* **openZro account.**
   Register one at [app.openzro.io](https://app.openzro.io/).

You would need to obtain a [setup key](/manage/peers/register-machines-using-setup-keys) to associate openZro client with your account.

The setup key could be found in the openZro Management dashboard under the Setup Keys tab - [https://app.openzro.io/setup-keys](https://app.openzro.io/setup-keys).
...
That is it! Enjoy using openZro.

If you would like to learn how to run openZro Client as an ECS agent on AWS, please refer to [this guide](#net-bird-client-on-aws-ecs-terraform).
