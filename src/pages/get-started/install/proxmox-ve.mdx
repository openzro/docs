...
Now that our system is updated, we can install openZro from the
signed APT repository (assuming an Ubuntu / Debian template — for
other distros see the [install guide](/get-started/install)):
...
Now that openZro is installed, we can connect our peer to the
deployment using the setup key we generated earlier. 
...
{/* TODO(docs): record openZro walkthrough — upstream had: <YouTube videoId="KMNS_JoHFhg" /> */}
