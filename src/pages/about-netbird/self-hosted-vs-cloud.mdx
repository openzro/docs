# Self-hosted vs. Cloud-hosted openZro

openZro can be used in two ways: self-hosted and cloud-hosted (SaaS). The self-hosted version is a standalone version
...
## Before Choosing to Self-Host openZro in Production

When we first created openZro, our goal was to simplify network administration — freeing admins from the headaches of
...
a lot happens behind the scenes. openZro relies on a range of interconnected components that work together to ensure secure
...
a reliable manner. openZro is not just one VPN server. You can read more about how openZro works [here](/about-openzro/how-openzro-works).

## What's New: No External IdP Required

Starting with version 0.62, self-hosting openZro has become significantly easier. Previously, self-hosting required setting up and maintaining a separate identity provider (like Zitadel, Keycloak, or Auth0). Now, the Management service supports **local user management** directly, which means:

- **No external IdP required** - Create and manage users directly in openZro
...
openZro platform is a combination of [multiple components](/about-openzro/how-openzro-works) that make it possible for
...
The cloud-hosted openZro only requires you to install the client software (openZro agent) on your machines and log them in to the network.
...
<Note>
SCIM provisioning and some enterprise features require a [Commercial License](https://openzro.io/pricing#on-prem) for self-hosted deployments.
</Note>

## Geo Distributed Relay Servers

openZro uses relay servers to establish connections between machines when a direct point-to-point connections isn't possible.
...
around the world ensuring that your machines can always establish a connection. The openZro agents pick the closest relay server.
...
- [Self-hosting Quickstart](/selfhosted/selfhosted-quickstart) - Get started with self-hosted openZro in 5 minutes
