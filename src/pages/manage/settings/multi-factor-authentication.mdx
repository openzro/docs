...
openZro MFA is primarily for users with **email / password login**. It is available for all users and can be enabled from the Dashboard on the Settings page [https://app.openzro.io/settings](https://app.openzro.io/settings).

<Note>
    You may not need openZro MFA. Your SSO Provider (Google, Microsoft etc.) may already have MFA enabled. Enabling this setting could result in duplicated MFA checks.
</Note>
<Note>
    This feature is only available in the cloud version of openZro. <br />Self-Hosted users can use their own Identity Provider for MFA.
</Note>
...
<Note>
    Currently openZro does not provide recovery codes for MFA. Make sure to have multiple administrators configured to prevent lockout situations.
