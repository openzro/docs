# openZro MSP Portal for Managed Service Providers

openZro Cloud offers a multi-tenant MSP Portal for Managed Service Providers (MSPs) and Managed Security Service Providers (MSSPs)
to manage customers (tenants) from a single place. As an MSP, you can manage multiple openZro accounts under your organization,
configure network settings, billing, and control network access for each tenant just like you would for your own openZro account.

An MSP account is a standard openZro account with the added 'Tenants' section, allowing for the management of customer networks.
With an MSP account, you can also manage your own internal network, just like with a regular openZro account.
...
* Ensure you have an active openZro subscription ([Team plan](https://openzro.io/pricing) or higher).
* Visit our MSP application form at [https://openzro.io/msp](https://openzro.io/demo?form=msp&utm_source=docs&utm_medium=docs&utm_content=msp-portal).
...
Once approved as an MSP, you will see the 'Tenants' section in your openZro dashboard.
...
<Note>
   Please see [Existing Account](#existing-account) if a openZro account already exists in our system for the supplied tenant domain.
</Note>
...
For your convenience, openZro provides a daily usage overview.
...
(see [openZro plans](/manage/settings/plans-and-billing#net-bird-plans) for more details).
...
to showcase openZro to your customers before committing to a paid plan. Simply select 'Continue with Free Trial' when
...
to demo openZro to companies that aren't yet your customers. However, if someone logs in using the same domain, openZro will
create a separate account to avoid any security risks. We hope that the ease of use and advanced security features of openZro will help drive
...
This is the recommended approach as it gives the MSP engineer a proper identity inside the tenant. Use openZro's [profile switching](/client/profiles) feature to easily switch between your MSP account and client tenant connections on the same device.

**Option 2: Add a Network and Routing Peer**

Add a openZro Network and a routing peer on the client tenant's infrastructure under the MSP account.