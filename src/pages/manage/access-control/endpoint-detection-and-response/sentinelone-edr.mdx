...
{/* TODO(docs): record openZro walkthrough — upstream had: <YouTube videoId="QVs0RhprVYM" /> */}
...
<Note>
Treat the API token securely and store it safely. You will need both the console URL and API token for the openZro integration configuration.
</Note>
...
<Note>
    openZro matches the SentinelOne agent to the peer using the Serial Number of the device. You must ensure that each of your devices has a unique serial number.
</Note>
