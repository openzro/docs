...
<Note>
    EDR / MDM posture checks ship in the BSD-3 management binary —
    no licensing gate. The driver code lives at
    [`management/server/mdm/`](https://github.com/openzro/openzro/tree/main/management/server/mdm),
    one file per vendor (~150 lines each). New vendors are
    welcome — see the [`Provider` interface](https://github.com/openzro/openzro/blob/main/management/server/mdm/provider.go)
    for the contract.
</Note>

## Supported EDR / MDM platforms
...
Tanium, Jamf Pro, and Kandji are on the
