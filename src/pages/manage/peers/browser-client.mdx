...
<Note>
    The Browser Client is a real openZro peer running entirely in your browser. The management server does not connect to target machines - all connections are made directly from your browser to the target peer through the openZro network.
</Note>
...
<Note>
    Before connecting via SSH, make sure that SSH Access is enabled both on the target peer and in the openZro Dashboard. Learn more about [enabling SSH access](/manage/peers/ssh).
</Note>
