...
* [Active openZro account](https://app.openzro.io/) with administrative access.
...
Select `Installer Package` to install openZro using the official macOS package. Using a package ensures you're installing the exact same version on all devices. This example uses the Apple Silicon package that you can download [here](https://pkgs.openzro.io/macos/arm64). Drag the file to the `Installer Package` field box to upload it to Kandji MDM.
