...
The openZro Kubernetes operator automatically creates [Networks and Resources](/manage/networks) in your openZro deployment, allowing you to
...
```
<Note>
  Replace `~/nb-pat.secret` with your openZro API key.
</Note>
...
```yaml
# Required: point this at your self-hosted openZro management server.
...
<Note>
 If a policy already exists with the name specified in `openzro.io/policy`, the other settings will be ignored in favor of the existing policy.
</Note>
...
...
```
With this setup, other peers in your openZro network can reach these pods using the domain `app.<OPENZRO_DOMAIN>`
(where `<OPENZRO_DOMAIN>` is the DNS suffix configured for your self-hosted deployment, e.g. `internal`). The access will be made using a DNS round-robin fashion for multiple pods.
...
<Note>
  Order of uninstallation is important; make sure to uninstall `openzro-operator-config` first to avoid orphaned resources. If you have any routing peers or policies in the cluster, uninstalling `kubernetes-operator` first may lead to issues.
</Note>
...
<Note>
  NBRoutingPeer deletion will be blocked if there are any Services with the annotation `openzro.io/expose: "true"` present in the cluster. Make sure to remove the annotation from those Services or delete the Services themselves before proceeding with the deletion of NBRoutingPeers.
</Note>
