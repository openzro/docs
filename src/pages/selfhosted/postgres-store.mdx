...
You can also pass the DSN as an `OPENZRO_` environment variable on the `openzro-server` container in your `docker-compose.yml`:

```yaml
environment:
  - OPENZRO_STORE_ENGINE_POSTGRES_DSN=host=<PG_HOST> user=<PG_USER> password=<PG_PASSWORD> dbname=<PG_DB_NAME> port=<PG_PORT>
```
...
<Note>
As of version 0.26.0, openZro used SQLite as its default database store. However, SQLite lacked support for cascading deletions,
