...
<Properties>
  ...
  <Property name="server.store.dsn" type="string">
    Connection string for postgres or mysql engines. For postgres: `host=localhost user=openzro password=secret dbname=openzro port=5432`. Alternatively, use the `NETBIRD_STORE_ENGINE_POSTGRES_DSN` or `NETBIRD_STORE_ENGINE_MYSQL_DSN` environment variables.
  </Property>
