...
<Note>
The `openzro-server` container combines what were previously separate management, signal, relay, and STUN containers into a single service. This simplifies the deployment architecture while maintaining the same functionality.
</Note>
...
|--------|---------------|-----------------|---------|
| `openzro-grpc` | `/signalexchange.SignalExchange/`, `/management.ManagementService/` | `openzro-server-h2c` (h2c scheme) | gRPC traffic for signal exchange and the management API. Uses HTTP/2 cleartext (h2c) backend because gRPC requires HTTP/2. |
...
<Note>
The STUN port (3478/udp) must always be exposed publicly, regardless of reverse proxy configuration. STUN uses UDP for NAT detection and cannot be proxied through HTTP reverse proxies. The STUN server is embedded in the combined `openzro-server` container and configured via the `server.stunPorts` field in `config.yaml`.
</Note>
...
<Properties>
  ...
  <Property name="server.dataDir" type="string">
    Data directory path where the server stores its database and state files. Default: `/var/lib/openzro`. Maps to the `openzro_data` Docker volume.
  </Property>
</Properties>
...
<Properties>
  ...
  <Property name="server.auth.cliRedirectURIs" type="array">
    Redirect URIs for CLI-based authentication. Default: `["http://localhost:53000/"]`. Used when authenticating via the `openzro` CLI tool.
  </Property>
...
<Note>
openZro also supports integration with external OIDC-compatible identity providers for Single Sign-On (SSO), Multi-Factor Authentication (MFA), and centralized user management. See the [Authentication & IdPs page](/selfhosted/identity-providers) for configuration details.
</Note>
...
<Properties>
  ...
  <Property name="server.store.dsn" type="string">
    Connection string for postgres or mysql engines. For postgres: `host=localhost user=openzro password=secret dbname=openzro port=5432`. Alternatively, use the `OPENZRO_STORE_ENGINE_POSTGRES_DSN` or `OPENZRO_STORE_ENGINE_MYSQL_DSN` environment variables.
  </Property>
...
<Properties>
  ...
  <Property name="server.activityStore.dsn" type="string">
    Connection string for postgres engine. Example: `host=localhost port=5432 user=postgres password=postgres dbname=openzro_activity sslmode=disable`.
  </Property>
...
<Properties>
  ...
  <Property name="server.authStore.dsn" type="string">
    Connection string for postgres engine. Example: `host=localhost port=5432 user=postgres password=postgres dbname=openzro_idp sslmode=disable`.
  </Property>
...
OPENZRO_MGMT_GRPC_API_ENDPOINT=https://openzro.example.com
...
|----------|-------------|
| `OPENZRO_MGMT_API_ENDPOINT` | The URL where the dashboard makes REST API calls to the management server (e.g., `https://openzro.example.com`). Must be accessible from users' browsers since API calls are made client-side. |
| `OPENZRO_MGMT_GRPC_API_ENDPOINT` | The URL for gRPC communication with the management server. Usually the same as the REST endpoint. Used for in-browser SSH and RDP clients, as well as surfacing management links for client config instructions. |
...
openzro-server:
  environment:
    - OPENZRO_STORE_ENGINE_POSTGRES_DSN=host=db-server user=openzro password=secret dbname=openzro port=5432
    # Or for MySQL:
    # - OPENZRO_STORE_ENGINE_MYSQL_DSN=user:password@tcp(host:3306)/openzro
```
...
<Note>
The default openZro deployment includes embedded relay, signal, and STUN services. External services are only needed for advanced use cases.
</Note>
