...
4. Use **Client ID** for `OPENZRO_AUTH_CLIENT_ID`
5. Use **Domain** to configure `OPENZRO_AUTH_OIDC_CONFIGURATION_ENDPOINT`:
...
6. Copy **Client ID** to `OPENZRO_AUTH_DEVICE_AUTH_CLIENT_ID`
...
8. Copy values:
   - **Client ID** → `OPENZRO_IDP_MGMT_CLIENT_ID`
   - **Client Secret** → `OPENZRO_IDP_MGMT_CLIENT_SECRET`
   - **Domain** → `OPENZRO_IDP_MGMT_EXTRA_AUDIENCE` (format: `https://<DOMAIN>/api/v2/`)
