...
<p>
    <img src="/docs-static/img/selfhosted/identity-providers/managed/microsoft-entra-id/6_openzro-config-filled-microsoft-entra.png" alt="openZro config filled" className="imagewrapper-big"/>
</p>
...
<Note>
Entra ID returns group **object IDs** (GUIDs) by default, not group names. Your openZro groups will appear as IDs like `a1b2c3d4-5678-90ab-cdef-1234567890ab`. To use display names instead, you need Azure AD Premium and cloud-only groups, or you can use openZro's [IdP sync (idp-sync)](/manage/team/single-sign-on/idp-sync) to resolve group names directly via the Microsoft Graph API.
</Note>
