...
<Note> 
While we maintain a list of 'supported' (tested) IdPs, **any OIDC provider should work with openZro's 'OIDC (Generic)' connector**.
</Note>
...
<Note>
    Any identity provider that supports OpenID Connect can be integrated with openZro. If your provider isn't listed above,
...
<Note> 
Groups with matching names in openZro and your IdP will **not** sync. To import a group from your IdP, first delete the existing group with that name in openZro.
</Note>

#### SCIM
openZro ships a [SCIM 2.0 server](https://github.com/openzro/openzro/tree/main/management/server/http/handlers/scim)
in the management binary — available in self-host with no licensing
gate. Tested against Okta and Microsoft Entra; setup walkthrough at
[Manage → Team → IdP Sync](/manage/team/idp-sync).
