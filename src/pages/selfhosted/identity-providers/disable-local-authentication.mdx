...
export const description = 'Disable local email/password authentication in your self-hosted openZro deployment so that all users must sign in through an external identity provider.'
...
<Note>
openZro will refuse to disable local authentication if no external identity provider connectors are configured.
