...
```bash
OZ_LOG_LEVEL=info
OZ_LISTEN_ADDRESS=:443
OZ_EXPOSED_ADDRESS=rels://relay-us.example.com:443
OZ_AUTH_SECRET=your-shared-secret-here

# TLS via Let's Encrypt (automatic certificate provisioning)
OZ_LETSENCRYPT_DOMAINS=relay-us.example.com
OZ_LETSENCRYPT_EMAIL=admin@example.com
OZ_LETSENCRYPT_DATA_DIR=/data/letsencrypt
...
services:
  relay:
    ...
    ports:
      - '443:443'
      # Expose all ports listed in OZ_STUN_PORTS
      - '3478:3478/udp'
    env_file:
      - relay.env
    volumes:
...
```bash
# Replace the OZ_LETSENCRYPT_* lines with:
OZ_TLS_CERT_FILE=/certs/fullchain.pem
OZ_TLS_KEY_FILE=/certs/privkey.pem
```
...
If deploying multiple relays (e.g., for different regions), repeat the steps above on each server. Use the **same `OZ_AUTH_SECRET`** but update the domain name for each.
...
<Warning>
The `secret` under `relays` and the `OZ_AUTH_SECRET` on all relay servers **must be identical**. Mismatched secrets will cause relay connections to fail silently.
</Warning>
