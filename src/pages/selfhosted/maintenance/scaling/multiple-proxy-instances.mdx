...
export const description =
    'Deploy multiple openZro proxy instances across separate servers for high availability, redundancy, and geographic distribution.'
...
<Note>
On additional servers that only run the proxy (not the full openZro stack), the Traefik configuration is simpler because there are no competing HTTP routers. All traffic on port 443 is passed through to the proxy container.
</Note>
