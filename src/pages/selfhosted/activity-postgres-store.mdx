...
OZ_ACTIVITY_EVENT_POSTGRES_DSN=host=localhost port=5432 user=<user> password=<password> dbname=<db_name>
```
...
```bash
OZ_ACTIVITY_EVENT_STORE_ENGINE=postgres
OZ_ACTIVITY_EVENT_POSTGRES_DSN="host=<PG_HOST> user=<PG_USER> password=<PG_PASSWORD> dbname=<PG_DB_NAME> port=<PG_PORT>"
```

4. Update `docker-compose.yml` file to  pass the postgres configuration to the `management` container
```yaml
environment:
  - OZ_ACTIVITY_EVENT_STORE_ENGINE=${OZ_ACTIVITY_EVENT_STORE_ENGINE}
  - OZ_ACTIVITY_EVENT_POSTGRES_DSN=${OZ_ACTIVITY_EVENT_POSTGRES_DSN}
env_file:
  - .env
```
...
2. Enable SQLite by setting the `OZ_ACTIVITY_EVENT_STORE_ENGINE` environment variable to `sqlite`:
```bash
OZ_ACTIVITY_EVENT_STORE_ENGINE=sqlite
```
