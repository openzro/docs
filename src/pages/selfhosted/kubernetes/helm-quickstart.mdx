...
signal:
  replicaCount: 3
  ingress:
    enabled: true             # signal stays on its own host (gRPC)
    className: nginx
    annotations:
      cert-manager.io/cluster-issuer: letsencrypt-prod
      nginx.ingress.kubernetes.io/backend-protocol: GRPC
...
## 4. DNS for the relay LoadBalancer

The relay Service comes up as `LoadBalancer` and the cloud
provider provisions an external IP. Update DNS:

```bash
kubectl get svc -n openzro openzro-relay
# NAME            TYPE           EXTERNAL-IP        PORT(S)
# openzro-relay   LoadBalancer   34.x.x.x           33080:30869/TCP
```

Point `relay.example.com` (the value you set in
`relay.publicHostname`) at that IP. cert-manager validates the
domain once DNS resolves and provisions the cert via the
`Certificate` CRD the chart emits.
