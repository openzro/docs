...
<Note>
This guide covers both the **combined container** setup (`openzro/openzro-server`, the default for new deployments) and the **multi-container** setup (separate `management`, `signal`, and `relay` images). Where commands or configuration differ between the two setups, both variants are shown.
</Note>
...
<Note>
The `HostSNI(*)` rule acts as a catch-all for any domain not matched by the existing openZro HTTP routers. The `priority=1` ensures this TCP router only handles traffic that no other router claims. Any domain pointing to your server that isn't `openzro.example.com` will be forwarded to the proxy.
</Note>
...
<Note>
Migrating from a standalone external IdP to the embedded IdP with your IdP as a connector requires user ID migration. See the [Migration Guide](/selfhosted/identity-providers#migration-guide-and-backwards-compatibility) or contact [support@openzro.io](mailto:support@openzro.io) for assistance.
</Note>
