...
Add STUN configuration to your relay service. You need to expose the STUN UDP port(s) in the `ports` section. You can configure multiple STUN ports if needed (e.g., for different network interfaces or load balancing) - ensure each port is exposed in `docker-compose.yml` and listed in `OZ_STUN_PORTS`:
...
```yaml
  relay:
    ...
    #   - OZ_EXPOSED_ADDRESS=rels://your-domain.com:443
    #   - OZ_AUTH_SECRET=your-relay-secret
    #   - OZ_ENABLE_STUN=true           # Enable embedded STUN
    #   - OZ_STUN_LOG_LEVEL=info        # STUN logging level
    #   - OZ_STUN_PORTS=3478            # STUN port(s), comma-separated for multiple (e.g., 3478,3479)
    logging:
      driver: "json-file"
      options:
        max-size: "500m"
...
OZ_LISTEN_ADDRESS=:80
OZ_EXPOSED_ADDRESS=rels://your-domain.com:443
OZ_AUTH_SECRET=your-existing-relay-secret

# New STUN configuration
OZ_ENABLE_STUN=true
OZ_STUN_LOG_LEVEL=info
OZ_STUN_PORTS=3478
# For multiple ports, use comma-separated values:
# OZ_STUN_PORTS=3478,3479
```

<Note>
Keep your existing `OZ_AUTH_SECRET` value from your current configuration. If using multiple STUN ports, ensure all ports are exposed in your `docker-compose.yml` and listed in `OZ_STUN_PORTS`.
</Note>
...
<Note>
Remove the `TURNConfig` section entirely as TURN is replaced by the openZro relay service.
</Note>
...
1. **Check STUN Server**:
   ...
   Enable debug logging for the STUN server by setting `OZ_STUN_LOG_LEVEL=debug` in your `relay.env`, then restart the relay service and observe the logs:
   ```bash
...
|----------|-------------|---------|
| `OZ_ENABLE_STUN` | Enable embedded STUN server | `false` |
| `OZ_STUN_PORTS` | STUN UDP port(s), comma-separated | `3478` |
| `OZ_STUN_LOG_LEVEL` | STUN server log level | `info` |
...
**Solution**: Verify `OZ_ENABLE_STUN=true` is set and the relay container has port 3478/udp exposed.
